...
Use shell commands or $ARGUMENTS to identify scope (pr/worktree/diff/repo).

## Step 2: Profile algorithmic complexity

Use search/list files to find:
```bash
# Find nested loops
...
For each finding, estimate impact at current scale, 10x, and 100x.

## Step 10: Generate scalability review report

Create `.ai/reviews/<SESSION_SLUG>/reviews/review-scalability-<YYYY-MM-DD>.md` with findings.
...
Create `.ai/reviews/<SESSION_SLUG>/reviews/review-scalability-<YYYY-MM-DD>.md`:
...
**Reviewer:** Codex Scalability Review Agent
**Date:** <YYYY-MM-DD>
...
**Required Changes:**
1. Add 2 database read replicas
2. Scale API servers 2x → 10x
3. Add Redis cluster
...
## Review Location
Saved to: `.ai/reviews/<SESSION_SLUG>/reviews/review-scalability-<YYYY-MM-DD>.md`
