...
`plan.md`, `implement.md`, and `auto.md` cite it instead of restating the boundary.
...
- **Human-gated run** (plan/implement/verify with a PO present): an intent-bearing decision is
  ASKED — a question in chat constructed per `_question-craft.md` (consequence-framed per W10.2).
