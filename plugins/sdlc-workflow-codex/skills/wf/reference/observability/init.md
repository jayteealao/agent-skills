...
lines, citing Step 1/2)**, then let the user confirm, refine, or replace it — **ask in chat and WAIT**, presenting
the real options with `Other (describe)` always available. These are **real forks with tradeoffs**, not a quiz.
Bias every option by the discovered stack and the ship-plan — never propose a JS default on a non-JS repo.
...
dashboards to stand up, provisioning ceiling, client/edge scope) and confirm before writing — ask in chat:
**Confirm** (write the contract), **Adjust** (re-run one decision), **Cancel** (discard).
