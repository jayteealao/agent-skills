...
Then confirm — ask in chat: **Implement all** · **Select items** (choose which gaps to close now) · **Cancel**.
...
- **Node** is the **reference adapter** — the richest realization (lift the patterns the old `setup-wide-logging`
  skill carried: pino/winston middleware, request-scoped builder, `res.on('finish')` emit, tail sampling).
...
Show the current-vs-desired diff / plan and the exact command, then **ask in chat** and WAIT — offer exactly
these choices:
- **Apply now** — run the command now. Requires the user to have authenticated separately; this command never
  enters a credential.
- **Print command only** (recommended) — write the exact command to the compliance artifact; mutate nothing.
- **Skip** — leave it unprovisioned.
