...
4d. **Apply the augmentation plan — author the augmentation artifacts shape decided.** Read `augmentations-needed` from `02-shape.md` frontmatter (if the field is absent or `[]`, skip this step). For each type listed:
   - Load `skills/wf/reference/augment/<type>.md` as a sub-procedure (e.g., `augment/instrument.md` for `instrument`).
   - Run only the artifact-authoring portion: write the corresponding artifact (`04b-instrument.md`, `04c-experiment.md`, `05c-benchmark.md` in baseline mode). Do NOT run the full standalone mode — plan's job here is to *author the artifact*, not run benchmarks end-to-end.
   - After authoring each artifact, record it in `00-index.md` `augmentations:` list with `status: ready`.
   **Augmentation order (when multiple):** `instrument` → `experiment` → `benchmark` (baseline) → `profile`. Instrument before experiment because experiment references instrument signals. Benchmark baseline before implement because compare needs it after.
...
- For each module the slice touches, trace **inbound callers** — search / list files in the repository for imports/requires of that module across the codebase
...
- Find all test files that cover the modules the slice will touch (search / list files in the repository for imports of affected modules in test files)
...
- **narrative:** the chat summary's lead paragraph, in the artifact's story voice — see [_narrative-voice.md](_narrative-voice.md). Same voice as the artifact's `## The Plan` section: relevance first, tradeoffs stated plainly, no `"This plan implements…"` openings. The router leads the chat summary with this paragraph; the fields below are the receipt beneath it.
- `slug: <slug>`
...
2. **Discovery phase (new plans only — skip for review-and-fix modes):**
   Before writing a plan, interview the user about implementation decisions that the shape and slice left open. Ask 8–12 questions across 2–3 rounds in chat (up to 4 per round), presenting options as a short numbered list.
...
**Compact recommended before proceeding** — planning research (alternatives, web searches, codebase exploration) is noise for implementation. Tell the user: "Consider running `/compact` before `$wf implement` — workflow state lives in the artifact files on disk and the SessionStart hook re-reads it automatically after compaction."
...
## The Plan
<!-- STORY SECTION — first, and self-sufficient. A reader who reads only this section understands what was produced, the load-bearing decisions and counts, and the top risk; the structured sections below are drill-down, not a substitute. Write it in the voice defined in `_narrative-voice.md` (Sebastian Raschka register: relevance first, why before how, tradeoffs stated plainly, varied rhythm — NO "This plan implements…" openings). 1–4 short paragraphs. -->
...
complete — raise it with the user in chat (present the three options as a numbered list: scope the
harness / author the proxy+deferral / PO-accept the risk) before writing the artifact. Verify's
Step 0 refuses to inherit an unresolved wall (`blocked-runtime-evidence-missing` routing, deferral
hatch unavailable), so skipping this gate only moves the stop later and makes it more expensive.
...
[`../../../references/fragments-gallery.html`](../../../references/fragments-gallery.html).
...
(`../../../components/`) runs after fragment
