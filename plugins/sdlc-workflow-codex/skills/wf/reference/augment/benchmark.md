...
**Mode (set by the calling stage, not a skill):**
...
|--------|--------|-------------|----------------|
| `<target>` | `+X%` | `<one-line: e.g., "new validation loop added in step 3 of plan">` | `<one-line: profile this target via the augment/profile.md sub-procedure, or accept if within acceptable range>` |
...
Re-baseline: re-run `plan` (it re-authors the baseline), or load this file in baseline mode
...
- **Not a profiler** — `wf-benchmark` measures aggregate time and allocations at the function/endpoint level. For flamegraphs and call-graph analysis, use the `augment/profile.md` sub-procedure.
...
- **Not a profiling substitute** — if a regression is found but the cause is unclear, run the `augment/profile.md` sub-procedure to find the hotspot before attempting to fix it.
...
`skills/wf/reference/_fragment-authoring.md` and follow
