...
- **`auto` never triggers rollback.** This phase is user-invoked only; there is no autonomous path
  to it. Automated canary/metric-gated rollback is explicitly out of scope.
...
Present the full runbook (rows, order, classes, mitigations), then ask the user directly in chat
presenting a short numbered list — identical discipline to ship Step 5:

```
Rollback runbook for run <run-id> (version <version> in <environment>). Execute?
  1. Go — execute the reversal steps in order, then verify.
  2. No-Go — abort. The runbook is preserved in 09-rollback-<run-id>.md as status: aborted.
```
