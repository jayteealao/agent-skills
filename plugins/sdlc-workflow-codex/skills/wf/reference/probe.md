...
Ad-hoc targets are not failures — they are data. The slice records them and the user can decide later whether to promote an ad-hoc target to a formal AC by adding it as a new slice (via `$wf intake <slug> <new scope>`).
