...
Before writing anything, present the proposed new slices for confirmation directly in chat:

Ask: "This will add the following new slice(s) to workflow `<slug>`: [list each: slug, goal, complexity, depends-on]. Existing slices will not be modified. Proceed?"

Present options as a short numbered list:
1. Confirm — Write the new slice files
