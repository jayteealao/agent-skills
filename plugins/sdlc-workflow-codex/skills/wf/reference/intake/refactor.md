...
---
```
Body: open with `## The Refactor` — the story section (arc per `../_story-arc.md`, 1–2 short paragraphs — the problem inherited, the decisions with reasons, the top open risk; no "This refactor implements…" opening) — then `## Target` (what), `## Why` (the structural problem), `## Frozen` (must-not-change APIs/behaviors), `## Target Structure`.
