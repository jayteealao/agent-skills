...
You are running `$wf intake fix`, a **compressed planning workflow** for small intentional changes.
...
| Escalate | If during planning the work no longer fits the `$wf intake fix` envelope, **warn and continue** — record the breach in the artifact and recommend `$wf intake <description>` for next time. Do not refuse. |
...
A "`$wf intake fix` envelope" section listing any tripwires that fired during planning. Tripwires are **warn-and-continue** — do NOT refuse to write the plan. Just record the breach so the user has the data to decide whether to keep going or restart with `$wf intake`. Tripwires:
...
> One or more `$wf intake fix` tripwires fired. The plan is still valid, but the work has grown beyond the compressed envelope. Consider restarting with `$wf intake <description>` for a full workflow next time. Run `$wf implement <slug>` to proceed with the current plan.
...
> ⚠ Plan is valid but exceeds the `$wf intake fix` envelope. Consider $wf intake for the next change like this.
...
- **Not a hotfix** — `$wf intake hotfix` exists for production incidents (forces production-branch base, requires diagnosis sub-agents, has tighter scope locks). Use `$wf intake hotfix` if there is an active incident.
- **Not a refactor workflow** — `$wf intake refactor` exists for behavior-preserving refactoring with test baselines. Use `$wf intake refactor` if the change is "make the code better without changing what it does."
