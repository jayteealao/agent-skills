...
You are running `$wf intake rca`, a **root-cause analysis workflow** that investigates an issue and recommends the right downstream skill, without writing a fix.
...
If the recommended next skill is `$wf intake fix` or `$wf intake hotfix`, still write `02-shape.md` — those skills ignore it, but it preserves the option for the user to switch routing to `$wf plan` later without losing the synthesis.
