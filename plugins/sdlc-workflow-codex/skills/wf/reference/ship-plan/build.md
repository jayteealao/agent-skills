...
Workflow artifacts and skill internals are private implementation context. Never expose them in external-facing outputs.
- Internal context includes workflow artifact paths (`.ai/workflows/...`, `.codex/...`, `.ai/dep-updates/...`, `.ai/ship-plan.md`), stage names or numbers, skill names, prompt/tooling details, control-file metadata, and private chain-of-thought or reasoning traces.
...
- **Fully runnable output.** Generated YAML must be syntactically valid and produce a working pipeline given the plan's secrets and skills. Use pinned action versions (`actions/checkout@v4`, etc.). Include `permissions:` blocks. Substitute the plan's literal `publish-cmd`, `publish-dry-run-cmd`, and `required-secrets[]` — not placeholders.
...
- It does not author or edit `.ai/ship-plan.md` — that is `$wf ship-plan init` / `$wf ship-plan edit`. This skill only *reads* the plan and closes the repo gap against it.
...
4. Search for files matching `.github/workflows/*.yml`. Read each file fully. Build an in-memory index: `filename → on-triggers → job-names → step-run-commands → secret-refs`. Secret refs are all `${{ secrets.<NAME> }}` patterns.
...
- **Missing:** check skill does not appear after the publish step.
...
- **Missing:** the skill does not appear in any PR workflow.
...
- Does it wire each planned hook in `git-hooks.hooks` (the `pre-commit` / `commit-msg` / `pre-push` skills)?
...
- **Non-compliant:** config present but a planned hook skill is not wired.
...
> Implement all missing/non-compliant items as listed above?
> 1. Implement all (Recommended) — create missing files; patch non-compliant files. No file is overwritten.
> 2. Select items — choose which gaps to close now; defer the rest.
> 3. Cancel — discard, make no changes.

If "Select items": present each non-Compliant audit row as a numbered list and proceed with only the selected set.
...
| other | Ask the user directly in chat: "What skill runs `<check-name>`?" |
...
For `test-coverage` with a `min-percent`, append the threshold to the skill if the tool supports it (e.g. `pytest --cov --cov-fail-under=<min-percent>`, `jest --coverage --coverageThreshold=...`); otherwise add a `# TODO: enforce <min-percent>% threshold` comment next to the run step.
...
**husky** (Node): ensure `package.json` `scripts.prepare: "husky"`; create `.husky/<hook>` files, each running the planned skills. Default wiring: `.husky/pre-commit` → `npx lint-staged`; `.husky/commit-msg` → `npx --no -- commitlint --edit "$1"` (only if `commit-convention.spec ≠ none`). Note that `husky` + `lint-staged` must be installed.
...
**pre-commit** (Python-friendly): create `.pre-commit-config.yaml` with repos/hooks matching the planned skills; note `pre-commit install` must be run.

**simple-git-hooks**: add a `simple-git-hooks` block to `package.json` mapping each hook to its skill.

If `lint-staged` is referenced and no `lint-staged` config exists, also write a minimal one from the format-check + lint skills in Block H.
...
- Add an exact install skill to `deps-to-install` in the compliance artifact (e.g. `npm i -D husky lint-staged @commitlint/{cli,config-conventional}`, `pipx install pre-commit`).
...
- `contributing-doc: true` and absent → create a `CONTRIBUTING.md` stub covering: bootstrap skill, how to run gates locally (the Block-H skills), commit-convention rules, and the PR process.
...
- **`apply-via: manual`** → never call the API. Write the ready-to-run `gh api -X PUT ... --input <payload>` skill into the compliance artifact and the chat return. Set `branch-protection-applied: printed`.
- **`apply-via: gh-api`** → show the current-vs-desired diff and the exact skill, then ask the user in chat:

> Apply branch protection to `<base>` on `<owner>/<repo>` now? This mutates the remote repository.
> 1. Apply via gh api (Recommended) — run the PUT now. Requires repo admin; needs `gh auth` with admin scope.
> 2. Print skill only — write the gh api skill to the compliance artifact; don't mutate the remote.
> 3. Skip — leave branch protection unchanged.

  - "Apply" → run the `gh api -X PUT` call. On success set `branch-protection-applied: yes`; on failure (e.g. 403 — not admin) print the error + the manual skill and set `branch-protection-applied: failed`.
  - "Print skill only" → set `branch-protection-applied: printed`.
...
Record `environment-protection-applied: <yes | printed | skipped | failed | n/a>`. Same rules: show diff + payload first, never silent, print-only writes skills to the compliance artifact.
...
For each of the three gated mutations, give the status and — when `printed`/`failed`/`manual` — the exact `gh api` skill + payload so the user can apply it by hand:
...
- `next-steps:`
  ...
  - For any `*-applied` field that is `printed`/`skipped`/`failed`, run the `gh api` skill from the compliance artifact (needs repo-admin auth)
