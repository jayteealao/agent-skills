...
> 1. Create seed template (Recommended) — write a markdown template with `{{version}}`, `{{project-name}}`, `{{release-url}}`, `{{changelog-summary}}` placeholders. `$wf announce` will fill these at release time.
> 2. Skip — I'll create it manually. The file must exist before `$wf announce` runs.
...
- `branch-protection` — `{ base-branch, required-checks[], required-approvals, dismiss-stale-reviews, require-up-to-date, enforce-admins, require-code-owner-reviews, require-conversation-resolution, require-linear-history, allow-force-pushes, allow-deletions, mechanism, apply-via }`. Pre-fill `required-checks[]` from Block H's derived pre-merge checks and `branch-protection-current`.
  ...
  - `apply-via` ∈ {`gh-api`, `manual`} — **whether `$wf ship-plan build` is permitted to apply these settings to the remote repo via `gh api` (behind its own confirm gate), or only print the skills.** Default `gh-api` only if the user explicitly opts in; otherwise `manual`.
...
If, during Steps 2 or 3, the user asks *"what does a typical X plan look like?"* — or if they pick a `template-hint` they're unfamiliar with — open the relevant file under `reference/ship-plan/ship-plan-templates/<kind>.md` and show the seed values as **reference reading**, not a fill-in form.
