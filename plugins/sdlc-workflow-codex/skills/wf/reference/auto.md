...
3. **Run the stage in-process.** Read `reference/<stage>.md` in full and execute it verbatim against `<slug>` (and the slice for per-slice stages), passing the same `$ARGUMENTS` the manual command would. The stage does its own work, writes its own artifact, and updates `00-index.md`. Do not summarize or shortcut it. Apply the grounded-progress rule in [_grounded-progress.md](_grounded-progress.md) to every progress claim the drive emits.
