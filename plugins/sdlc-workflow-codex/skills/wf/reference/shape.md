...
3. **Check prerequisites:**
   ...
   - If `current-stage` in the index is already past shape → WARN: "Stage 2 (shape) has already been completed. Running it again will overwrite `02-shape.md`. Proceed?" Ask the user in chat.
...
- What code **calls into** the affected area? (callers, consumers, dependents — search / list files in the repository for imports/requires of affected modules)
...
- **Ask the user directly in chat** for multiple-choice PO questions (risk tolerance, appetite, structured decisions), presenting options as a short numbered list. Use freeform chat for open-ended questions (behavior, acceptance criteria, non-goals). Append every answer to `po-answers.md` with timestamp and stage.
...
**Rules:**
- Ask exactly 20 questions across 5 rounds of 4 in chat, presenting each round's options as a short numbered list.
...
- Each option should describe a concrete direction. Allow the user to answer "Other" or freeform when needed.
- Indicate whether multiple options can coexist or they're mutually exclusive.
