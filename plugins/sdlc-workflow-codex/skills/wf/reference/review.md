...
4. **Present for triage** — ask the user directly in chat, presenting each finding as a short numbered list with its options (Fix / Defer / Dismiss). Follow the same protocol as Step 4b below, but only show `deferred` and `untriaged` findings.
...
5. **Check prerequisites (workflow-type-aware AND review-scope-aware):**
   ...
   - **Change-mode** (`workflow-type: fix` / `hotfix`): the compressed-lifecycle's **un-suffixed single-slice** standard files (`03-slice.md`, `04-plan.md`, `05-implement.md`, optional `06-verify.md`) + the lead `01-<mode>.md` (`01-fix.md` / `01-hotfix.md`). Exactly one slice; `selected-slice` is its slug. Review as standard mode with the un-suffixed names. (`review-scope: slug-wide` — one `07-review.md`.) For **hotfix**, review defaults to the `security` rubric (`$wf review <slug> security`); widen only if the change warrants it.
...
# Step 4b: Triage ALL Findings

After deduplication, present ALL deduplicated findings to the user for triage. Ask the user directly in chat presenting options as a short numbered list. This gives the user explicit control over what gets fixed, deferred, or dismissed. **`Fix` decisions execute in Step 4c — they are no longer deferred to a separate `$wf implement reviews` invocation.**
...
4. If the sub-agent could not fix, record `COULD NOT FIX: <reason>` — this counts toward `convergence: escalated`.
