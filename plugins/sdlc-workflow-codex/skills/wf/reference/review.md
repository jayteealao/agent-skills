...
1. **Exact slug match** — `.ai/workflows/<token>/00-index.md` exists → **stage mode**. Continue with
   the stage logic below (TRIAGE MODE / Step 0 orient / the accumulating-ledger dispatch). The optional
   second token is `<slice>` or `triage`, exactly as before.
2. **`sweep` or a known dimension/aggregate key** (no slug matched) → **ad-hoc mode**. Jump to the
   `# Ad-hoc review (no slug)` section at the end of this file. Dimension keys and aggregate keys are
   listed there.
...
Each command maps to `review/<name>.md` — **except** the two design dimensions `design-audit` and `design-critique`, which map to `design/audit.md` and `design/critique.md` respectively (the relocated design library; see the "design work" selection rule below).
...
that were previously `resolved`, and any prior `open` (untriaged) findings. **Skip
`pre-existing: true` findings** — they are not this change's triage burden; they land in
`## Pre-existing Debt` with a suggested `$wf intake` routing, and the user acts on them there (a
user may still ask to fix one in-run, but the default flow never prompts for it). Findings already triaged
