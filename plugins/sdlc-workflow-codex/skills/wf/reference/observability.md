...
There is no `edit` sub-command yet — amend `.ai/observability.md` by hand (it is a plain markdown contract, like
`.ai/ship-plan.md`). If `audit` routing surfaces repeated contract friction, an `edit` sub-key is the sanctioned
next addition.
