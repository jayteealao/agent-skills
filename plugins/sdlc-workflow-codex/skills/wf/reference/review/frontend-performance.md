...
Before reviewing frontend performance, ask the user directly in chat:
...
- `.map`, `.filter`, `.reduce` in render return paths

## Step 4: Check image optimization

Search/list files for:
- Large PNG/JPG assets in the public directory
- `<img` elements without `loading="lazy"`
- `<img` elements without `width=` attribute (CLS risk)

## Step 5: Review code-splitting

Search for:
- `React.lazy` or `lazy()` usage
- `<Route` without associated lazy imports
...
Create `.ai/workflows/<SESSION_SLUG>/reviews/review-frontend-performance-<YYYY-MM-DD>.md` with findings.
...
Create `.ai/workflows/<SESSION_SLUG>/reviews/review-frontend-performance-<YYYY-MM-DD>.md`:

```markdown
---
skill: $review frontend-performance
...
**Scope:** <Description>
**Reviewer:** Codex Frontend Performance Review Agent
**Date:** <YYYY-MM-DD>
...
## Review Location
Saved to: `.ai/workflows/<SESSION_SLUG>/reviews/review-frontend-performance-<YYYY-MM-DD>.md`
