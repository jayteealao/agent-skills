...
### IAM Scan
Search/list files for:
- `aws_iam_policy` or `aws_iam_role_policy` resources in Terraform
- `"*"` on Resource fields in IAM policies
- `AdministratorAccess`, `roles/owner`, or `Contributor` strings
...
### Network Scan
Search/list files for:
- `aws_security_group` and `ingress` blocks
- `0.0.0.0/0` CIDR ranges
...
### Secrets Scan
Search/list files for:
- `password`, `api_key`, `secret` values in terraform or k8s files
- `kind: ConfigMap` blocks with password/API key values
- `master_password` or `admin_password` fields
...
### Encryption Scan
Search/list files for:
- `aws_s3_bucket` or `aws_db_instance` resources
- `encryption` or `kms_key` configurations
- Port 80 or plain HTTP protocol references
...
### Logging Scan
Search/list files for:
- `aws_cloudtrail` resources
- `audit` or `logging` configuration blocks
...
Create `.ai/workflows/<SESSION_SLUG>/reviews/review-infra-security-{YYYY-MM-DD}.md`
...
## Review Location
Saved to: `.ai/workflows/{SESSION_SLUG}/reviews/review-infra-security-{YYYY-MM-DD}.md`
