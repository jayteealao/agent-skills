...
Standard session inference from `.ai/reviews/README.md` (last entry).

## Step 1: Load session context

1. Validate `.ai/reviews/<SESSION_SLUG>/` exists
2. Read `.ai/reviews/<SESSION_SLUG>/README.md` for context
...
2. **TARGET** (if not provided)
   - If SCOPE is `pr`: need PR URL — ask the user directly in chat
...
Use shell commands and git to get:
...
Create `.ai/reviews/<SESSION_SLUG>/reviews/review-style-consistency-{YYYY-MM-DD}.md`
...
**Date:** {YYYY-MM-DD}
**Reviewer:** Codex Style Consistency Review Agent
...
## Review Location
Saved to: `.ai/reviews/{SESSION_SLUG}/reviews/review-style-consistency-{YYYY-MM-DD}.md`
