...
- `<div.*onClick` in components directory
- `<span.*onClick` in components directory
- `<input` without associated labels
- `role=` without accompanying `aria-` attributes
...
Search for:
- `useNavigate`, `navigate`, `router.push` calls
- `Modal`, `Dialog`, `Drawer` component usage
- `outline:.*none` in CSS files
...
### Forms Scan
Search for:
- `<input`, `<textarea`, `<select` usage
- `error`, `invalid`, `required` in form components
...
Search for:
- `loading`, `isLoading`, `pending` state variables
- `Toast`, `Notification`, `Snackbar` components
...
### Third-Party Scan
Search for:
- Imports from `@mui`, `antd`, `@chakra-ui`
...
Create `.ai/workflows/<SESSION_SLUG>/reviews/review-frontend-accessibility-{YYYY-MM-DD}.md`
...
```markdown
---
skill: $review frontend-accessibility
...
## Review Location
Saved to: `.ai/workflows/{SESSION_SLUG}/reviews/review-frontend-accessibility-{YYYY-MM-DD}.md`
