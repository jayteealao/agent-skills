...
Before reviewing performance, ask the user directly in chat if the answers are not clear from context:
...
Use shell commands to search/list files:
```bash
# Find nested loops
grep -rn "for.*for" src/
...
Create `.ai/workflows/<SESSION_SLUG>/reviews/review-performance-{YYYY-MM-DD}.md`
...
**Reviewer:** Codex Performance Review Agent
**Date:** {YYYY-MM-DD}
...
## Review Location
Saved to: `.ai/workflows/{SESSION_SLUG}/reviews/review-performance-{YYYY-MM-DD}.md`
