...
Both `missing` and `drift` **gate**: present the situation and require an explicit decision. Ask in chat with a short numbered list and wait for the answer.
...
- **Caller = `ship`** — ship literally reads the plan; it cannot run without one. Ask in chat, two options only, and wait:
  "No ship plan at .ai/ship-plan.md. Ship is plan-driven and cannot run without it. Author one now?
  1. Create it — STOP here; run `$wf ship-plan init --from-template <kind>`, then re-run `$wf ship`.
  2. Cancel — abort this ship run; leave everything unchanged."
...
- **Caller = `handoff`** — a repo may legitimately ship outside this workflow (CI/CD auto-deploy on merge, release owned elsewhere, `branch-strategy: none`). Offer the not-applicable path. Ask in chat and wait:
  "No ship plan at .ai/ship-plan.md. `$wf ship` will require one. Author it now, or is shipping handled outside this workflow?
  1. Create it now — STOP; run `$wf ship-plan init --from-template <kind>` before shipping, then re-run handoff.
  2. Shipping is external — this work ships outside `$wf ship` (auto-deploy/owned elsewhere). Proceed; record `ship-plan-readiness: not-applicable`.
  3. Cancel — abort the handoff; leave everything unchanged."
...
**When `amendable` is non-empty** — ask in chat and wait:

"The ship plan drifted from the repo (<N> gating finding(s)). Amend it before continuing?
1. Amend now and continue — amend block(s) <blocks> here, re-check drift, and carry on in THIS run; no restart.
2. Amend separately — STOP; run `$wf ship-plan edit` yourself, then re-run this stage.
3. Acknowledge and proceed — the drift is known/intentional. Record a reason and continue on the current plan.
4. Cancel — abort; leave everything unchanged."

Name in options 1 and 2 **only the blocks the amendable findings point at.** A `merge`-class finding riding along in the same table does not add its block to the amendment — that is how an amendment gets asked to clear something it cannot.

**When `amendable` is empty — never offer an amendment.** Every surviving finding clears on `merge` or `repo`, so an amend option here proposes a `plan-version` bump that changes nothing, re-opens the plan-scoped ledger, and guarantees this question comes back. Ask in chat and wait, offering only the actions that can end it:

"The ship plan is <N> finding(s) out of step with the branch, none of which amending the plan can clear. How do you want to proceed?
1. Acknowledge and proceed — record a reason. The entry is branch-scoped: it survives plan amendments and does not re-ask for the life of this branch.
2. Fix in the repo — STOP; run <the repo action named by the finding>, then re-run this stage.
3. Cancel — abort; leave everything unchanged."

Drop option 2 when no finding is `clears-on: repo` — a two-option gate is honest; a third option that matches no finding is noise. If the user still wants to amend the plan for its own sake, that is `$wf ship-plan edit` on their own terms, not this gate's remedy; say so in the acknowledgement's advisory line rather than offering it as a way out of the gate.
