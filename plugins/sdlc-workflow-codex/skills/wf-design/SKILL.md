...
Load the matching register reference: `reference/brand.md` or `reference/product.md`.
...
**Agent 4 — Surface ranger.** Find files relevant to the current task using your native file-listing tools: active page/component files, related CSS, primary component. If working in workflow context, read `02-shape.md` for `files-in-scope`.
...
Load the reference for the resolved sub-command from `reference/<sub-command>.md` and follow it verbatim. Do not summarize, paraphrase, or skip.
