...
- **One artifact, in the existing workflow.** Write `.ai/workflows/<slug>/03-slice-refactor-<descriptor>.md` (collision suffix `-2`, `-3` if needed). Frontmatter: `type: slice`, `slice-slug: refactor-<descriptor>`, `slice-type: refactor`, `compressed: true`, `origin: wf-quick/refactor`, `stage-number: 3`, `status: defined`.
- **Same content, different home.** Body carries the same sections the standalone refactor would have written to `01-refactor.md` (target identification, test baseline, behavior-preservation invariants, refactor plan, verification plan), under a `# Compressed Slice: refactor` heading with a one-line provenance preamble.
- **No new workflow, no new branch, no `01-refactor.md`, no new top-level `00-index.md`.** The slug already owns those.
- **Index updates:** append the slice file to `00-index.md.workflow-files`, append `{slug: refactor-<descriptor>, slice-type: refactor, created-at: <iso>}` to `00-index.md.compressed-slices` (create the array if missing). If `.ai/workflows/<slug>/03-slice.md` exists, also append `{slug, status: defined, slice-type: refactor, compressed: true}` to its `slices`, bump `total-slices`, update `updated-at`. Do not modify `current-stage`, `selected-slice`, `status`, `branch`, or `progress`. Also rewrite the `updated-at` column on `<slug>`'s row in `.ai/workflows/INDEX.md` (see SKILL.md Step 1 step 6).
- **Chat return:** one line — `wf-quick refactor → compressed slice refactor-<descriptor> on <slug>` — plus the recommended next step (`$wf implement <slug>` to apply the refactor, or `$wf-meta status <slug>` to inspect).
...
| Resume | Pass an existing refactor slug to resume from the last completed step |
...
3. **Branch check (MANDATORY):**
   ...
   - Refactors SHOULD use a dedicated branch. Ask the user directly in chat, presenting as a short numbered list: "1. Create dedicated branch, 2. Use current branch".
...
Launch parallel sub-agents. Work sequentially unless the user explicitly requested parallel execution.
...
- Read every file that imports or calls the refactoring target — search for imports across the entire codebase. These are all the callers that must still work after the refactor
...
Prompt with ALL of the following:
- Find all test files that cover the refactoring target — search for imports of the target modules in test directories
...
**If coverage gaps are significant:** Ask the user directly in chat, presenting as a numbered list:
1. Add tests first (recommended)
2. Proceed with gaps noted as risk
3. Abort — I need to understand coverage better first
...
**After writing:** Confirm the plan with the user by asking directly in chat, presenting as a numbered list:
1. Proceed step by step
2. Adjust plan (describe changes)
3. The plan is too large — help me scope it down

# Step 4 — Implement
Execute each plan step. **One step at a time — never combine steps.**

For each step:
1. Note the step as in-progress.
2. Make only the changes described in this step. Nothing else.
3. Run the verify command for this step (from the plan). If it fails:
...
4. If green: **commit the step**: `refactor(<slug>): step <N> — <short description>`
5. Note the step as completed.
...
4. **Caller check:** Search for imports of the refactored modules across the codebase. Verify all callers still compile/parse. For dynamic languages, spot-check key call sites.
