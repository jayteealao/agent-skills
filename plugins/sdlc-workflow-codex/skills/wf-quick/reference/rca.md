...
You are running `wf-rca`, a **root-cause analysis workflow** that investigates an issue and recommends the right downstream skill, without writing a fix.
...
- **Same content, different home.** Body carries the same sections the standalone rca would have written to `01-rca.md` (parallel-diagnosis findings, root-cause synthesis, downstream-skill recommendation), under a `# Compressed Slice: rca` heading with a one-line provenance preamble.
...
- **Chat return:** one line — `wf-quick rca → compressed slice rca-<descriptor> on <slug>` — plus the recommended downstream skill (e.g., `$wf-quick refactor <slug> <target>`, `$wf-quick hotfix <slug> ...`, `$wf-meta amend <slug>`), still scoped where the rca says it should go. Use the positional-slug form (slug as the first argument after the sub-command) — there is no `--slug` flag in v9.10.0+.
...
# Pipeline
`1·symptom` → `2·investigate` → `3·synthesize` → `$wf plan` | `$wf-quick fix` | `$wf-quick hotfix`
...
| Next | `$wf plan <slug>` (default — non-trivial fixes), `$wf-quick fix <slug>` (small fixes), `$wf-quick hotfix <slug>` (active production incidents). The artifact recommends one based on the diagnosis. |
...
4. **Read project context (lightweight):**
   ...
   - List `.ai/workflows/*/00-index.md` filenames to spot any related active workflows the symptom might be tied to.

# Step 1 — Symptom intake
Ask at most **3 questions** — stop as soon as you have enough to investigate. Present as a numbered list in chat:
...
recommended-next: <$wf plan|$wf-quick fix|$wf-quick hotfix|human-triage>
...
**Direction, not a plan.** 1 to 3 lines naming the area and the approach. Do NOT enumerate implementation steps — that belongs in `$wf plan` or `$wf-quick fix`. Examples of the right shape:
...
| Suggested fix shape touches ≤3 files, ≤5 steps, no new dependency, no architecture change | `$wf-quick fix <slug>` |
...
If the recommended next skill is `$wf-quick fix` or `$wf-quick hotfix`, still write `02-shape.md` — those skills ignore it, but it preserves the option for the user to switch routing to `$wf plan` later without losing the synthesis.
...
is absent). Author them here, now, while the incident is still in context.
...
Full contract:
[`fragment-author-contract.md`](../../../references/fragment-author-contract.md).

### Use `@include` for shared chrome (v9.20.1+)

The fragment is **body-only**: `rca.mjs` already emits the heading and the metric-row,
and draws the timeline + causal-chain figures (suppressing its static copies when
the fragment is present). Do **not** repeat the metric-row in the fragment — start
at the interactive timeline:
