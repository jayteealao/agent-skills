...
# Step 2 — Research
For each package that needs updating, launch parallel web research sub-agents. Group packages into batches of 3–5 to avoid over-parallelization. Work sequentially unless the user explicitly requested parallel execution.
...
**After writing:** Present a summary to the user and ask them to confirm before implementing. Ask directly in chat, presenting as a numbered list:
1. Proceed with full plan
2. Proceed with P0 security updates only
3. Audit-only — save plan, do not implement
