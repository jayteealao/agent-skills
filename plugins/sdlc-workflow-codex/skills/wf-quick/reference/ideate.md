...
- **Same content, different home.** Body carries the same sections the standalone ideate would have written to `01-ideate.md` (lens summary, ranked opportunities with rationale, top-N recommendations and follow-up skills), under a `# Compressed Slice: ideate` heading with a one-line provenance preamble.
...
- **Chat return:** one line — `wf-quick ideate → compressed slice ideate-<descriptor> on <slug>` — plus the top-ranked idea and its recommended next skill (e.g., `$wf-quick refactor <slug> <area>`, `$wf intake <description>`). Use the positional-slug form (slug as the first argument after the sub-command) — there is no `--slug` flag in v9.10.0+.
...
- If you catch yourself starting to implement an idea, STOP. This skill discovers work; it does not do it.
...
- Find dead code: exported functions/classes with no imports elsewhere. Search for import counts.
...
- Have a proposed entry point (which `wf-*` skill starts this work)
...
Each idea above maps directly to a `$wf intake` skill invocation. Copy the entry skill for any idea you want to pursue. The slug suggestion is a starting point — you can adjust it.
