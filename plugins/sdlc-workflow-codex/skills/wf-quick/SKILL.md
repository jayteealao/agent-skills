...
3. **Slug detection on the second token (positional, no flag).** This is the only way to opt into slug-mode in v9.10.0 — there is no `--slug` flag.
   - Read `.ai/workflows/INDEX.md` (the global workflow registry; format documented in the wf-meta sync reference).
   ...
   - **If `INDEX.md` exists** → take the second token (the first arg after the sub-command). Run an anchored search: look for an exact match on the slug column (first tab-separated field, `^<token>\t`).
     ...
     - **Hit AND status column = `closed`** → ask the user directly in chat: *"Workflow `<slug>` is closed. Append a compressed slice anyway?"* — wait for explicit confirmation. On yes → slug-mode; on no → STOP.
...
- Source the descriptor from the sub-command's first descriptive argument when available (e.g., `$wf-quick rca X "payment spike"` → `rca-payment-spike`). If no descriptive arg or the arg is generic, fall back to `<sub>-<utc-date>` (e.g., `ideate-2026-05-13`).
...
**Format (compact) — standalone mode:**
...
**Format (compact) — slug-mode (compressed slice):**
