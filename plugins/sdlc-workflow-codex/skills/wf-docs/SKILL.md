...
- Internal context includes workflow artifact paths (`.ai/workflows/...`, `.ai/dep-updates/...`, `.codex/...`), stage names or numbers, skill names, sub-agent names, prompt/tooling details, control-file metadata, and private chain-of-thought or reasoning traces.
...
3. **Generate run ID** for orchestrator mode: `docs-<YYYYMMDD-HHMM>` (run `date +"%Y%m%d-%H%M"` via your native shell tools).
...
- Find all markdown files in the project: `README.md`, `docs/`, `CONTRIBUTING.md`, `CHANGELOG.md`, `wiki/`, API docs, embedded docstrings, and any other `.md` files using your native file-listing tools
...
status: complete
created-at: <real timestamp via shell>
---
```
...
If the audit surfaced ambiguous classifications (a doc that mixes quadrants, or a request that could be tutorial-or-how-to), load `reference/plan.md` to apply the Diátaxis decision table before recording the action.
...
**After writing:** Present a summary and confirm with the user by asking them directly in chat, presenting options as a short list:
```
...
1. Record the action being taken.
2. **Read required source files** before writing anything — do not write from memory.
3. **Load the matching primitive reference** from `reference/<primitive>.md` and follow it verbatim. The primitive references are:
...
4. **Write or update the file** at the target path using your native file-editing tools.
5. **For delete actions:** confirm with the user one more time before deleting. Never delete silently.
6. Record the action as completed.
...
Spot-check the generated documentation for quality and coherence. Load the docs review reference (key: `review`) for the Diátaxis-discipline rubric.
...
1. Load `reference/<primitive>.md` in full.
...
| `review` | `reference/<key>.md` | Audit existing docs against Diátaxis principles with prioritized fixes |
...
- **Timestamps must be real:** run `date -u +"%Y-%m-%dT%H:%M:%SZ"` via your native shell tools.
