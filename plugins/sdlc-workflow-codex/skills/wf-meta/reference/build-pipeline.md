...
- **Trace insertions.** When adding to an existing file, add a single-line comment above the inserted block: `# Added by wf-meta build-pipeline — plan v<N>, <YYYY-MM-DD>`. Never add this comment to content the user wrote themselves. For non-YAML config files use that language's comment syntax; for files that cannot carry comments (JSON), record the provenance in the compliance artifact instead.
...
4. **Search for `.github/workflows/*.yml`.** Read each file fully. Build an in-memory index: `filename → on-triggers → job-names → step-run-commands → secret-refs`. Secret refs are all `${{ secrets.<NAME> }}` patterns.
...
Then ask the user in chat, presenting the options as a short numbered list:
...
| other | Ask the user in chat: "What command runs `<check-name>`?" |
...
**If a PR workflow exists but is missing jobs** — use your native file-editing tools to append the missing jobs at the end of the `jobs:` block. Prefix the block with: `# Added by wf-meta build-pipeline — plan v<N>, <YYYY-MM-DD>`.
...
- **`apply-via: gh-api`** → show the current-vs-desired diff and the exact command, then ask the user in chat (present as a numbered list):
...
> 2. Print command only — write the gh api command to the compliance artifact; don't mutate the remote.
...
Skip if no `security` block. For each non-`none` gate (align generated steps to the conventions in `../../review/reference/supply-chain.md`):
