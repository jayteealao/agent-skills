---
description: Generate a dense, token-efficient context brief for a workflow. Reads all stage files and po-answers, distills into ~500 words. Designed for resuming after a break, onboarding into a new session, or recovering context.
argument-hint: [slug]
---
...
You are running `wf-resume`, the **context recovery** skill for the SDLC lifecycle.

# Pipeline
1·intake → 2·shape → 3·slice → 4·plan → 5·implement → 6·verify → 7·review → 8·handoff → 9·ship → 10·retro

This skill reads the full workflow trail and produces a dense brief. It writes `90-resume.md` as a reference artifact, then returns the brief in chat.
...
1. **Resolve the slug** from `$ARGUMENTS`. If no slug given:
   - **If `.ai/workflows/INDEX.md` exists** (v9.11.0) → read it; filter rows where the status column is not `closed`. If exactly one non-closed row → use it. If multiple → list them as `slug — status — updated-at` (one row per workflow, sourced from INDEX.md) and ask the user which one in chat, presenting as a short numbered list. Note: this picker drops `current-stage` from the prior listing (INDEX.md doesn't carry it) — pass an explicit slug if you want the full per-workflow read.
   - **If `INDEX.md` does NOT exist** → fall back to searching `.ai/workflows/*/00-index.md` for active workflows; same one-of/multiple/none logic as before. Append a one-line tip to the final chat return: *"Tip: run `$wf-meta sync` once to bootstrap `.ai/workflows/INDEX.md` — enumeration becomes registry-driven."*
...
3. **Read every existing stage file** listed in `workflow-files` (or search `.ai/workflows/<slug>/*.md`). For each, parse YAML frontmatter and scan the body for:
...
6. **Check slice progress** (if slices exist):
   - Search for all `03-slice-*.md`, `04-plan-*.md`, `05-implement-*.md`, `06-verify-*.md` files
...
3. **Return the brief in chat.** The chat output IS the brief — no preamble, no "here's your brief", no explanation. Just the brief itself, so the user (or anyone reading the context) gets maximum signal with minimum noise.
