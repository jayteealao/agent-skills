...
This skill does NOT advance any workflow. It is purely read-only — no files are written, no state is changed.
...
- If you catch yourself about to modify anything, STOP. This skill is read-only.
...
1. **Enumerate workflows** (v9.11.0):
   ...
   - **If `INDEX.md` does NOT exist** → fall back to searching for all `.ai/workflows/*/00-index.md` files in the project. Also include this one-line tip in the final chat return: *"Tip: run `$wf-meta sync` once to bootstrap `.ai/workflows/INDEX.md` — enumeration becomes registry-driven and skips deleted dirs cleanly."*
...
- `runtime-evidence-deferrals` (list of `{slice, reason, deferred-at, cleared-by}` entries; field may be absent on older workflows)
...
If the user is clearly trying to resume work (e.g., returning after a break), end with a suggestion: "Run `$wf-meta next <slug>` for routing details, or the next skill directly."
