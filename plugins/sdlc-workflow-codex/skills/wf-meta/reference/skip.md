...
2. A **minimal stub** for the skipped stage's expected artifact — so downstream skills that check for `02-shape.md`, `03-slice.md`, etc. do not error on a missing prerequisite
...
- If skipping `verify`: WARN in chat: "Skipping verification means no test results, no acceptance criteria confirmation, and no observable signal that the implementation worked. Are you sure? (yes to proceed)"
- If skipping `review`: WARN in chat: "Skipping code review removes the safety check before handoff. Are you sure? (yes to proceed)"

Present the warning in chat and wait for the user to reply. If the user confirms, proceed. If not, STOP.
...
4. **Check if stage is already skipped or complete:**
   - Read `00-index.md` `progress` field. If `<stage>: skipped` or `<stage>: complete` already → WARN in chat: "Stage `<stage>` is already marked `<status>` in this workflow. Skipping again will overwrite the skip record. Proceed? (yes to continue)"
5. **Check stage ordering:**
   - Read `current-stage` from `00-index.md`.
   - If the requested skip stage is MORE THAN ONE stage ahead of `current-stage` → WARN in chat: "You are at `<current-stage>` but skipping `<stage>` which is `<N>` stages ahead. Stages between them will also be effectively skipped — they will remain incomplete. Run `$wf-meta skip <slug> <intermediate-stage>` for each intermediate stage, or confirm you want to jump `<N>` stages. (yes to proceed)"
...
**Stub artifact written:** `<artifact-path>` — downstream skills will find this file but it contains no real content.
```

# Step 4 — Write stub artifact

Write a minimal stub for the stage's expected artifact. The stub signals to downstream skills that the stage was intentionally skipped, not accidentally omitted.

**For `review` skip:** The stub filename is `07-review-<slice-slug>.md`. Resolve the slice from `selected-slice-or-focus` in `00-index.md` (or a third argument, if the user passed one). If no slice is resolvable, STOP and ask the user in chat which slice the skip applies to — there is no longer a single workflow-wide review file to skip. Add `slice-slug: <slice-slug>` to the stub frontmatter.
...
Downstream skills that read this file should treat its absence of content as intentional. Do not error or block on missing sections — proceed with the context available from prior stage artifacts.
```
...
3. `next-command` → the skill for the new `current-stage`
...
> Warning: <stage> skipped. Quality gate bypassed — proceed with caution.

If multiple intermediate stages would be effectively incomplete due to a jump, add:

> Warning: Stages between `<current>` and `<skipped>` remain incomplete. Run `$wf-meta skip <slug> <stage>` for each or accept the gap.
