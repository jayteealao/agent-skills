...
If close reason was not provided in `$ARGUMENTS`, ask the user directly in chat, presenting the options as a short numbered list:

> Why is this workflow being closed?
> 1. `cancelled` — work is no longer wanted
> 2. `superseded` — replaced by another approach or PR
> 3. `deferred` — valid but not now, may resume later
> 4. `completed-externally` — done outside this workflow
> 5. `merged-into` — absorbed into a larger workflow or PR

Record the answer. If `superseded` or `merged-into`, also ask in chat: "What superseded/absorbed it? (PR URL, workflow slug, or description)"
...
> To resume: run `$wf intake <description>` with the same description, or restore `status: in-progress` in `00-index.md` and run `$wf-meta resume <slug>` to pick up where this left off. All prior artifacts are intact at `.ai/workflows/<slug>/`.
...
> Workflow deferred. To resume: `$wf-meta resume <slug>` or `$wf intake <description>`.
