...
This is a **utility skill**, not a pipeline stage. It adds net-new slices to any workflow — whether in-progress or fully complete — without touching existing slice artifacts. It does NOT correct existing slices (use `$wf-meta amend`) and does NOT fix bugs in existing implementation (use `$wf implement`).
...
6. **Read source artifacts** based on mode:
   - `from-review`: search for and read every `07-review-*.md` file in the workflow directory (one per reviewed slice). Aggregate full findings, triage decisions, and recommendations across all of them — extension candidates often emerge from siblings' reviews.
...
Ask 4–8 targeted questions in chat (up to 4 per round, presenting options as a short numbered list where applicable) to define the new slices precisely.
...
Before writing anything, present the proposed new slices for confirmation in chat:
...
- Options (as a numbered list):
  1. Confirm — create slices as described
  2. Revise — I want to adjust the slice definitions
  3. Cancel — don't add any new slices
