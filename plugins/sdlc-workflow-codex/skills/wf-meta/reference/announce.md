...
This is a **utility skill**, not a pipeline stage. It is designed to run after `$wf ship` (stage 9) completes with a `go-nogo: go` or `conditional-go` decision. It can also run after `$wf retro` (stage 10).
...
- If you catch yourself about to fix code or advance the workflow, STOP. This skill writes communication copy only.

# Step 0 — Orient

1. **Resolve the slug** from `$ARGUMENTS` (first argument). If no slug is given, infer the most recent shipped workflow from `.ai/workflows/*/00-index.md` (look for `status: complete` or `current-stage: ship` or `current-stage: retro`). If ambiguous, ask the user directly in chat, presenting the options as a short numbered list.
...
- `../../wf-docs/reference/how-to.md`
- `../../wf-docs/reference/reference.md`
- `../../wf-docs/reference/tutorial.md`
- `../../wf-docs/reference/explanation.md`
- `../../wf-docs/reference/readme.md`
...
Ask the user in chat about audience and delivery channel (both in a single message):

- **Audience question:** Which announcement audiences to generate. Options should reflect what this specific feature warrants — a minor fix may only need engineering; a user-visible feature needs users + product. Offer "All three" as the default. Present as a numbered list: 1. Engineering only, 2. Product only, 3. Users only, 4. All three.
- **Channel question:** Where each announcement will be sent. This shapes tone and length. Present as a numbered list: 1. Slack/chat (short, scannable, can use emoji), 2. Email (longer, formal, prose), 3. GitHub Release (markdown with headers and code blocks), 4. Internal wiki/Notion (structured, permanent record). **Pre-fill the description with the plan's `announcement.channels[]` when present** (e.g., *"Plan specifies: #releases, release-notes@example.com"*) so the user can confirm rather than re-enter.
...
**Announcement template:** If `announcement.template-path` was set in the plan and the file exists, use it as the structural scaffold for each draft — substitute `{{version}}`, `{{project-name}}`, `{{release-url}}`, `{{changelog-summary}}` from the ship-run artifact. If the file does not exist, proceed without it and note the missing template in the chat return.
...
- `announcements-sent` → append the list of channels that announcements were drafted for (e.g., `["#releases", "release-notes@example.com"]`)
- `updated-at` → current ISO 8601 timestamp

Do NOT change any other field. This feedback closes the ship→announce loop: the run artifact now records which channels received the announcement, visible to retro analysis.
...
- `ship-run-updated: <latest-run-path> (announcements-sent populated)` — or `skipped: legacy shape` if applicable
