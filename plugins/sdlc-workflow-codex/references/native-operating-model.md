...
Do not create lifecycle artifacts for ordinary tasks. Run a structured SDLC workflow only when the user asks for one (`$wf …`, `$wf-quick …`), the work will span threads, or continuity would otherwise be lost.
