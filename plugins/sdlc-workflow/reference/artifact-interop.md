...
The `.ai/` directory tree is the **shared source of truth** for all SDLC workflow state. Neither host maintains a parallel JSON store — every read, resume, and recommendation is derived by reading these artifacts directly from disk.
...
Every workflow artifact carries `schema: sdlc/v1` in its YAML frontmatter. Both hosts read this field to confirm an artifact belongs to this system before acting on it.

## Cross-thread continuity

Continuity across sessions — on either host — comes from reading artifacts directly:
...
The `next-command`, `next-invocation`, and `recommended-next-*` fields written into `00-index.md` by stage executors are written in the neutral `/wf …` spelling (for example `next-invocation: /wf implement <slug> <slice>`); each host substitutes its own sigil per `skills/wf/reference/_host-invocation.md`.

Producer provenance (which host wrote an artifact) is diagnostic-only. Either host reads and extends artifacts regardless of which host wrote them — the artifact format is the contract, not the producer.

## Workflow lease

Mutating commands (any stage that writes to `.ai/workflows/<slug>/`) acquire the workflow lease at `.ai/workflows/<slug>/.locks/workflow.json` before writing and release it after. Read-only commands (`/wf status`, `/wf recap`, and so on) take no lease.
