...
- **Distinction from `/wf-quick`.** `/wf-quick` *starts* new workflows (intake, RCA, hotfix, etc.); `/wf-meta` *navigates and manages* existing workflows. Aside from `/wf-quick intake` (which begins the canonical lifecycle) and `/wf-meta resume` (which re-enters an ongoing one), the two skills don't overlap.
