...
- **Channel question:** Where each announcement will be sent. This shapes tone and length. Options: Slack/chat (short, scannable, can use emoji), Email (longer, formal, prose), GitHub Release (markdown with headers and code blocks), Internal wiki/Notion (structured, permanent record). **Pre-fill the description with the plan's `announcement.channels[]` when present** (e.g., *"Plan specifies: #releases, release-notes@example.com"*) so the user can confirm rather than re-enter.
