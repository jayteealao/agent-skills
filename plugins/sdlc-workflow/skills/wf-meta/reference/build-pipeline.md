...
- It does not author or edit `.ai/ship-plan.md` — that is `init-ship-plan` / `amend ship-plan`. This command only *reads* the plan and closes the repo gap against it.
...
Skip if no `security` block. For each non-`none` gate (align generated steps to the conventions in `${CLAUDE_PLUGIN_ROOT}/skills/review/reference/supply-chain.md`):
