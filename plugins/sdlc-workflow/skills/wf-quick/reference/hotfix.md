...
**Model for every dispatched agent:** `sonnet`. REQUIRED on every `Task` call. Both agents do causal reasoning under time pressure — Root Cause must trace symptom-to-cause across recent changes, Impact must reason about blast radius. Haiku's pattern-matching is insufficient for an incident-mode root-cause hunt; Opus is too slow. Sonnet 4.6 is the right tier.
