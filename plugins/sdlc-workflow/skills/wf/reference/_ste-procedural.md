...
**Scope.** These rules apply to text that a reader executes or that leaves the
workflow: every document `/wf docs` or the diataxis skill writes, the
structured sections of every workflow artifact, reader-visible text in view
fragments (both tiers), runbook and how-to steps, user-facing product copy
(labels, errors, empty states), and external-facing outputs (release notes,
PR bodies, announcements). They do NOT apply to artifact story sections —
those follow [_narrative-voice.md](_narrative-voice.md), which wins wherever
the two contracts conflict. When one document contains both (a how-to with an
intro, an artifact with a story above its structured sections), each passage
follows the contract for its own kind of text.
