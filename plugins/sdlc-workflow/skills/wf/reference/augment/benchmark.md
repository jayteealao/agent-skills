...
- **Not a profiling substitute** — if a regression is found but the cause is unclear, run the `augment/profile.md` sub-procedure (or `/wf probe`) to find the hotspot before attempting to fix it.
