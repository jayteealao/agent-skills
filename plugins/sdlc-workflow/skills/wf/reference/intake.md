...
1. **Slug + mode keyword → compressed slice.** If `token0` exactly matches an existing `.ai/workflows/<token0>/00-index.md` on disk **AND** `token1` is in the mode keyword set *(except `adopt`, `amend`, `modernize`)* → **slug-mode**. Consume `token0` as `<slug>` and `token1` as `<mode>`; the rest are the mode's instructions. The run will attach as **one compressed slice** on that workflow (Step 4). **If `token1` is `adopt`**, this is not slug-mode —
   ...
   *"`adopt` roots a new workflow from the current diff; it can't attach to an existing slug. Run `/wf intake adopt` (no slug), or `/wf intake <slug> <scope>` to extend `<slug>` with new work."* *(An exact on-disk slug match is an intentional attach and does not trigger the collision prompt in `_intake-context.md`.)*
...
Propose **at most one** mode, **once**, as a gate question per [_gate-question.md](_gate-question.md) offering the proposed mode (recommended) vs "Plain intake (default)". On accept, load that mode reference standalone; on decline, `intake/default.md`. Nothing routes into a build-committing mode (`fix`, `hotfix`, `refactor`, `update-deps`) without the user's yes.
...
The mode decides how far the flow travels. Run only the stages the mode needs. This is the single mode→span map:
...
| `brainstorm` | **terminal thinking loop** — roots a `type:workflow-index` slug with the `01-brainstorm` board only; no branch | compressed slice (the board in the slice body) | terminal → **stays open**; only the person's `done` leaves the loop; `brainstorm <slug>` resumes; retire via `/wf close <slug>` |
