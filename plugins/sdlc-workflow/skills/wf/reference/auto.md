...
- **It removes inter-stage friction, not intra-stage gates.** Each delegated stage keeps its own user gate: `verify` owns its `AskUserQuestion` Fix/Skip/Escalate loop, `review` owns its Fix/Defer/Dismiss triage. `auto` does **not** suppress, replace, or pre-answer any of them. When a stage asks the user a question, the user answers it in the normal way and the stage continues; `auto` resumes the chain afterward.
