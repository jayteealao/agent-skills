...
- **Consults at the designated gates (free CLI, by objective trigger)** — `consult` is model-invocable, so at the plan, review, and diagnosis gates `auto` drives, the model **auto-invokes** `/consult codex …` (pinned to free `codex`/`claude`) whenever that stage's objective trigger fires — a carried intent-risk, a ship-with-caveats verdict, an inferred-not-observed AC, a risk-bearing surface (the names live in [_consult-triggers.md](_consult-triggers.md)). A stage with no trigger present adds no consult; the paid REST oracles are never fanned out unattended.
