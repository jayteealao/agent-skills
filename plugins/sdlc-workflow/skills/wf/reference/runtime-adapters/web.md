...
- **Web Vitals (Gap 8 fix):** After driving each criterion, capture Core Web Vitals — Largest Contentful Paint (LCP), Cumulative Layout Shift (CLS), and Interaction to Next Paint (INP). Use whatever measurement mechanism the driver supports: the browser's Performance API via script evaluation, Lighthouse, Playwright's built-in tracing, or a web-vitals library injected into the page. Record the values as structured data. INP > 200 ms is a HIGH issue; LCP > 2500 ms is WARN; CLS > 0.1 is WARN.
