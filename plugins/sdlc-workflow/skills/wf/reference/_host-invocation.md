...
| Design canvas (design stage) | When the session lists the Artifact tool: call it with `action: quickstart` and `intent: design`, create the Design artifact from the type it names, follow the type's own instructions, draw one artboard per surface and state, and record the link as `canvas:` in `02c-craft.md`. Otherwise use the Codex column | `/imagery` for the north-star comps; offer `/uiproto` for an interactive prototype when `externalDispatch.enabled` | Same as Codex |
| Design system sync (`/wf design sync`) | When the session offers the `/design-sync` skill: hand off to it, one component at a time, after the person approves the plan | None; report that no sync surface exists | None; report that no sync surface exists |
