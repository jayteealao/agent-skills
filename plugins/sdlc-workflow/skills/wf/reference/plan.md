...
| Conditional inputs (mandatory when present) | `02b-design.md` (design brief), `02c-craft.md` (the visual contract the person confirmed at the design stage; the plan carries explicit steps for every mock fidelity inventory item and the implementation contract) |
...
5. **Read design context (mandatory when design is needed per [design/_lane.md](design/_lane.md)).** Otherwise skip this step. Load `design/_design-context.md` for the register, shared design laws, absolute bans, and the motion/interface-detail summary; when the feature touches motion, interface detail, or typography, also load `animate.md` / `polish.md` / `typeset.md`. Use only those sections; the preflight/image/mutation sections govern `/wf design`. Then:
...
6. **Design gate.** Apply the human rule in [design/_lane.md](design/_lane.md): when design is needed and not settled, STOP with `status: awaiting-input` and route to `/wf design <slug>`. Plan never authors `02c-craft.md`. Write `## Design Components` per the lane's `plan` duty.
7. **Apply the augmentation plan.** Read `augmentations-needed` from `02-shape.md` frontmatter (absent or `[]` → skip). For each entry, load its sub-procedure and run only its artifact-authoring mode, then record it in `00-index.md` so `implement` / `verify` consume it. Run this after the research sub-agents return. Order when multiple: `instrument` → `experiment` → `benchmark` → `profile`.
...
2. **Discovery phase (new plans only; skip for review-and-fix modes).** Before writing a plan, interview the user about implementation decisions shape and slice left open: as gate questions per [_gate-question.md](_gate-question.md), ask only the questions this plan needs, in as few rounds as the dependencies allow, up to 4 per round. Every question is about how to build this slice and cites files, modules, patterns, and tradeoffs the sub-agents found. Do not re-ask anything decided in shape or slice; pre-fill known decisions and confirm only when findings contradict them. Present genuinely different approaches, not a right answer with decoys. Later rounds build on earlier answers. Apply [_question-craft.md](_question-craft.md) to every question. Ask about the implementation approach (competing patterns, extend versus write new, which library API), sequencing and dependencies, test strategy (coverage gaps, unit/integration/E2E, new patterns), and risk (deprecations, version mismatches, advisories, shape assumptions the codebase contradicts, spikes). Append every answer to `po-answers.md` with timestamp and `stage: plan`.
...
3. **Single plan mode (new):** run the research sub-agents and the freshness research; run the discovery phase; if `augmentations-needed` is set, author the augmentation artifacts now (Step 0 item 7); produce a minimal execution-ready plan; write `04-plan-<slice-slug>.md`; update the master `04-plan.md`.
...
- **Option A (default): Implement** → `/wf implement <slug> <slice-slug>`. The plan is complete. Recommend compacting the session first; the SessionStart hook re-reads the artifacts after compaction.
