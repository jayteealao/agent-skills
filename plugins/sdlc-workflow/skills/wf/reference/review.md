...
4. **Present for triage via AskUserQuestion** — follow the same protocol as Step 4b below, but only show `deferred` and `open` findings. (A `Fix` decision here may also run the Step 4c fix loop on those findings.)
...
5. **Record the outcome ON the finding** — set its `status` and `fixed-at = now` everywhere it
   ...
   - sub-agent could not → `status: could-not-fix` (stays OPEN; still counts against the verdict),
     and `TaskUpdate(... description: "COULD NOT FIX: <reason>")`.
...
**Compact recommended before re-invoking** — fix sub-agent chatter and triage UI is noise for the next review pass. Tell the user: "Consider running `/compact` before re-invoking — workflow state lives in the artifact files on disk (the merged ledger is in `07-review-<slice-slug>.md`) and the SessionStart hook re-reads it automatically after compaction."
