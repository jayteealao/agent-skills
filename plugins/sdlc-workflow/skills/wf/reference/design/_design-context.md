...
| Brief | For `craft` only: design brief authored and explicitly confirmed by the user | `craft` authors the brief inline as its first step (`design/craft.md` Step 0, following `design/shape.md`); confirm it before the contract. There is no separate `shape` command. |
