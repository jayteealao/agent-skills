...
| Effort tier → host setting | Pass `model:` on the call: low, medium and high = `sonnet` (`haiku` is not used; re-evaluate when Haiku 5.5 ships). Never `opus`, and never omit the pin — a child must not inherit the parent's model | Pass the reasoning effort on the spawn: low, medium, high | Pass `model:` on the call with the same tier aliases as Claude Code: low, medium and high = `sonnet`. Never omit the pin |
