...
| `thread-contested` | at `done`, a live brainstorm thread is party to an `open` tension | intake |
| `claim-contradicted` | a bounded read contradicted a brainstorm statement (`check: contradicted`) and the person kept the thread live | intake |
