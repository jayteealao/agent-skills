...
| Produces | `01-investigate.md` (problem + architecture map + 2–3 option sketches with tradeoffs), `00-index.md`. **No `02-shape.md`** — the user chooses an option first; the downstream command (`/wf intake` or `/wf intake fix`) does the shape pass on the chosen option. |
