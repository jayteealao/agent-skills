...
4. **Determine workflow source mode** from `workflow-type`:
   ...
   - `workflow-type: fix` / `hotfix` → **change-mode (compressed standard lifecycle).** The mode authored the planning half as STANDARD, single-slice, **un-suffixed** files: `01-<mode>.md` (`type: intake`; `01-fix.md` / `01-hotfix.md`), `02-shape.md`, `03-slice.md` (`type: slice-index`, one slice), `04-plan.md`. There is exactly **one** slice and `selected-slice` on the index is its slug. Implement exactly as **standard mode** with one substitution: every per-slice file is **un-suffixed** — read `04-plan.md` and write `05-implement.md` (NOT the `-<slice-slug>`-suffixed files of multi-slice standard mode, and NOT the single `01-quick.md` of compressed mode). Wherever a step below names a `-<slice-slug>`-suffixed file, use the un-suffixed name for change-mode. (hotfix's `07-review` defaults to the `security` rubric.)
   ...
   - `workflow-type: rf` (refactor) / `workflow-type: docs` → **alternate workflows**. These have their own implement stages and should NOT be using `/wf implement`. STOP and direct the user to the workflow's own implement command (e.g., `/wf intake refactor`).
...
7. **Read the source context by mode:**
   ...
   - **Change-mode** (`fix` / `hotfix`): same as standard but **un-suffixed**, plus the lead:
     - `01-<mode>.md` (`type: intake`; `01-fix.md` / `01-hotfix.md`) — the compressed brief + acceptance criteria (hotfix's lead also carries the `## Diagnosis`)
...
8. **Read augmentation context (optional — workflow may have any combination):**
   ...
   **Read design planning artifacts** (separate from augmentations):
   ...
   - **Applying design transforms (deepest design consumer — when `stack.ui ≠ ∅`).** The references above are *read-only judgment context* for a normal implement pass. But when this implement pass is the implement step of a `/wf design` transform (the dispatcher drives slice→plan→**implement**→verify), `implement` *applies* the design: read the transform's playbook from `skills/wf/reference/design/<name>.md` and apply it during the build, then **register it as a `design-<sub>` augmentation** in `00-index.md` and write the `design-notes/<sub>-<timestamp>.md` artifact (contract in `reference/design.md` Step 5). A transform no longer needs pre-existing code — it either creates the surface or modifies the slug's existing implementation, both here. Gate: if `stack.ui` is empty, there is no design transform to apply — skip.
