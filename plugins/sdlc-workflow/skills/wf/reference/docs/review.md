...
Do not add a self-check pass on top of this rubric. In orchestrator mode this rubric runs inside `docs.md` Step 5's fresh-context review sub-agent; in a standalone invocation the dispatching session gives this rubric to a fresh-context sub-agent the same way. The fresh context is the objectivity check.
