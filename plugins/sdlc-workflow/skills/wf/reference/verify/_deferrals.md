...
`verify.md` Step 7.5 applies this file when a user-observable AC has no runtime evidence and no reachable rung can produce it. Deferral is a last resort: it is honest only after the constraint-resolution ladder ([../runtime-adapters.md](../runtime-adapters.md)) was climbed and each rung's outcome recorded. Defer only the residual that no rung can reach.
