...
Read the workflow's intake, shape, and plan artifacts to learn the change's intent. Take the diff scope from the dispatch prompt, per `_stage.md`. Hunt defects with the checklist in this file. Estimate each finding's impact at 10x and at 100x the current scale. Record each finding with the evidence that the non-negotiables require.
