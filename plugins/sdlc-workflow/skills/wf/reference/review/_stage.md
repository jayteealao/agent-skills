...
> **Auto second opinion (verdict-gated).** After findings are merged and the verdict is derived, **auto-invoke** `/consult codex review <scope>` (pinning `codex`/`claude` keeps it free) when ANY of the [_consult-triggers.md](../_consult-triggers.md) triggers holds: `verdict-ship-with-caveats` (the caveat verdict *is* the trigger, so fire it rather than listing it in next-steps) or `blocker-fixed-in-loop` (a blocker finding was fixed in-loop this run and the re-check verdict flipped to ship). Record each run in the review artifact's `consult-runs:` frontmatter. A clean `ship` with no fixed-in-loop finding, or an obvious block, adds no consult. The user may invoke it with any provider (`user-invoked`).
