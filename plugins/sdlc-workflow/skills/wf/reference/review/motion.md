...
Create `.claude/<SESSION_SLUG>/reviews/review-motion-<YYYY-MM-DD>.md` with findings.

## Step 6: Update session README

```bash
echo "- [Motion Review](reviews/review-motion-$(date +%Y-%m-%d).md)" >> .claude/<SESSION_SLUG>/README.md
```

# OUTPUT FORMAT

Create `.claude/<SESSION_SLUG>/reviews/review-motion-<YYYY-MM-DD>.md`:

```markdown
---
command: /wf review motion
...
**Scope:** <Description>
**Reviewer:** Claude Motion Review Agent
**Date:** <YYYY-MM-DD>
...
## Review Location
Saved to: `.claude/<SESSION_SLUG>/reviews/review-motion-<YYYY-MM-DD>.md`
