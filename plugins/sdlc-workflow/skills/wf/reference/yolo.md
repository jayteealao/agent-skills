...
- **Self-managed build** — `update-deps`. It self-authors `05`/`06` and routes to `/wf review` (not `/wf implement`), so `yolo` **does not** drive it — orientation blocks with `route='/wf intake update-deps <slug>'`.
