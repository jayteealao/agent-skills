...
Negotiating the environment, precisely: an unattended driver that can only *wait* on an obstacle will wait forever, so before any deferral `yolo` climbs the **env-remediation rung** — the ladder's named step for changing the environment's configuration until a rung becomes reachable. Its boundary is authority, not effort. Reversible, run-scoped changes to state the run owns are taken unprompted and recorded: rebind a harness-owned service to a free port (an emulator suite on 8081 is the same evidence as one on 8080 — **never defer over a port the run itself binds**), boot the documented headless mode, start an AVD that already exists, run a provisioning script the repo ships, execute an install the plan authorized. Changes to state the run does *not* own are surfaced, never improvised: killing a process the run did not start (another user's server holding a port is not the driver's to reclaim), mutating host or system configuration, or editing product code to make evidence collectible. That last one is the sharp edge — when the wall turns out to be `code-owned` (the repo's own hard-coded port, host, endpoint, or fixture), the driver may **not** quietly patch product code to unblock itself, and it may **not** defer either: it surfaces the wall as a decision, because "our own constant blocks our own evidence" is scoping work the PO owns, not an environment fact to route around. A wall classified `code-owned` with no scoped resolution is a HARD-STOP, not an auto-deferral — which is precisely the failure this rung exists to prevent, after one slug stacked five slices of deferred live evidence behind a port its own debug build had hard-coded.
