...
One hook is a Claude Code mod: `hooks/mod/register.ts`, a function-hooks module that adds the `/wf` picker. With `CLAUDE_CODE_ENABLE_FUNCTION_HOOKS=1` set, typing `/wf` lists every key in the typeahead as `/wf-<key>`, and a `/wf` that still needs a key, a slug, or a slice draws a numbered list above the prompt; a digit picks a row, the wheel turns the page, and a filter field narrows the rows. The last pick fills the prompt; you press Enter to run it. The same module draws a workflow strip above the prompt (stage, slice, next step, cost), offers the next invocation as the prompt's Tab suggestion after a stage turn, toasts when a stage ends without its artifact, counts intake and shape questions in the dialog, shows the driver heartbeat during auto and yolo, names the stage in the spinner, reports the hub under the logo, and opens a `/wf-dashboard` pane; each aid has a switch in `/config`. Without the flag the module does not load and `/wf` works as before.
