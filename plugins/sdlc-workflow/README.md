...
> **v9.43.0** closes the gaps the v9.42.0 pipeline left open. **Generated artifacts are now actually wired**: branch protection sets `require_code_owner_reviews` so `CODEOWNERS` is enforced (not cosmetic), `amend` re-derives the pre-merge check lists when code-quality gates change, and `build-pipeline` warns that hooks/CI are inert until dev-deps are installed. The branch-protection payload gained full fidelity (code-owner / conversation-resolution / linear-history / force-push & deletion locks, `mechanism: branch-protection | ruleset`, stronger-than-plan guard). New **Block K — security & supply-chain gates** (CodeQL, dependency-audit, secret-scanning, SBOM, license policy) plus extensions for per-environment GitHub protection, merge controls, and CI ergonomics (caching/matrix/concurrency). `build-pipeline` now applies **three** gated remote settings (branch protection, environment protection, merge settings). Plan blocks A–K; audits A–S.
>
> **v9.42.0** makes `/wf-meta init-ship-plan` + `/wf-meta build-pipeline` cover the **entire** developer-experience + CI + CD pipeline, not just the outbound release. The ship plan gains three inbound blocks — **H** (code-quality gates: format/lint/type-check/coverage + commit-message and PR-title convention), **I** (local developer experience: git hooks, `.editorconfig`, runtime-version files, task runner, bootstrap), and **J** (repo governance: branch protection, CODEOWNERS, PR/issue templates, dependency automation). `build-pipeline` grows Audits **K–O** to build all of it — commitlint + PR-title CI, local git hooks, dev-experience files, governance files — and, for the first time, can apply **branch protection** to the remote repo via `gh api`, but only when the plan opts in (`apply-via: gh-api`) and only behind an explicit confirm gate. Existing Blocks A–G and Audits A–J are unchanged. See [Author a ship plan](docs/site/guides/releasing.html) and the [Ship-plan schema](docs/site/guides/releasing.html).
>
> **v9.19.0** adds a uniform **Final Summary Contract** to every router (`/wf`, `/wf-quick`, `/wf-meta`, `/wf-design`, `/wf-docs`, `/review`). Every sub-command of every router now ends with a predictable 8-line summary: what ran (verb-first), which artifacts were created, 1–3 lines of key facts (verdict, counts, decisions, tripwires), and a concrete `Next:` invocation (or `Done` for terminal stages). Previously summaries were ad-hoc — some sub-commands had explicit "Chat return contract" sections in inconsistent shapes, others had no final step at all. The fix is architectural: the contract lives in each router SKILL.md and the dispatcher applies it to every sub-command it loads. References that already specify chat-return content keep their *values*; the router contract supplies the *shape*. Per-router specializations: `/wf-design` adds `Register:` and `Image gate:`; `/wf-docs` adds `Files: <created> | <updated> | <deleted> | <skipped>` (orchestrator) or `Quadrant:` (primitive); `/review` adds `Verdict:` and per-severity `Findings:` counts.
>
> **v9.18.0** renames the `/wf-quick quick` sub-command to `/wf-quick fix`. The behavior — compressed planning that collapses intake/shape/design/slice/plan into one artifact — is unchanged; only the sub-command name moves. `/wf-quick quick` now returns a redirect message (parallel to how `intake` and `docs` redirects worked in v9.1.0 and v9.4.0). Existing pre-v9.18.0 artifacts on disk (`workflow-type: quick`, `01-quick.md`) remain readable in resume mode and `/wf implement`'s compressed-mode prerequisite check. New artifacts use the `fix` namespace: `01-fix.md`, `workflow-type: fix`, branch `fix/<slug>`, slice-type `fix`. The argument shape is the same: `/wf-quick fix <description>`.
>
> **v9.17.1** (copy-only, no behavior change) refreshes all 11 skill descriptions using Diátaxis's reference voice — verb-first, no "This skill should be used when…" preamble, no inline sub-command enumeration where `argument-hint` already carries it, no version-history fragments or internal lingo in user-facing copy. Worst-case before/after: the `/wf-quick` description went from ~190 words (10-item enumeration with inline definitions + `(v9.10.0):` + `type: slice` lingo) to ~60 words. The `router-metadata.json` description for each router was synced to match.
>
> **v9.17.0** finishes propagating **explicit per-Task model tiering** across every fan-out dispatch site in the plugin (a cost optimization that's invisible to users except for the bill). The Task tool's `model` parameter is now REQUIRED at every sub-agent dispatch — no silent inheritance from the parent session's model. Three patterns: rubric reviewers and bounded extractors (most fan-outs) run on **Haiku 4.5**; judgment-heavy work (architecture, refactor-safety, security in `/review`; `investigate`, `rca`, `hotfix` in `/wf-quick`; all four fix-loops in `/wf review` / `/wf verify` / `/wf implement reviews`) runs on **Sonnet 4.6**; synthesizers and workflow-aware explainers (`/wf-meta how` Modes B-synth, C-synth, D, E; `/review sweep` synthesis) **inherit parent**. The largest single delta: `/wf-meta how --research` (Mode C) — 6-8 research agents per question move from parent-inherited to Haiku. Resolution is data-driven where possible: `models.default` + `models.overrides` blocks live in `skills/<router>/router-metadata.json` for `review`, `wf-meta`, and `wf-quick`; the static verifier (`scripts/verify-router-migration.mjs` Check 4) enforces "every dimension resolves to a valid model". `scripts/migrate-router.mjs` now preserves `models` across regenerations and skips frontmatter-less reference files (was destroying the former, crashing on the latter). The `/wf` skill (verify functional 1-5, plan/shape/implement Explore agents, ship freshness, retro analysis, experiment/benchmark/instrument) is the named follow-up. See CHANGELOG entries for v9.16 and v9.17.
>
> **v9.11.0** wires four more read paths into `.ai/workflows/INDEX.md` (the global registry from v9.10.0). `/wf-meta status` (no-arg) enumerates from the registry and skips deleted directories cleanly; `/wf-meta resume` and `/wf-meta next` source their multi-workflow pickers from the registry (`slug — status — updated-at`). `/wf intake` greps the registry before creating, prompting on duplicate slugs (open → resume/amend/pick-new; closed → pick-new/reopen — closed slugs cannot be reused). `/wf` gained a centralized **Step 0.5 fuzzy-suggest** in `skills/wf/SKILL.md` — typo'd slug args across the 13 slug-consuming sub-commands surface "Did you mean `<slug>` (closed)?" via Levenshtein-1 + substring match. All four read paths fall back to the v9.10.0 behavior (glob + no collision check + no fuzzy-suggest) when INDEX.md is absent, with a one-line tip suggesting `/wf-meta sync`. See `skills/wf-meta/reference/{status,resume,next}.md`, `skills/wf/reference/intake.md` Step 0 sub-step 2, and `skills/wf/SKILL.md` Step 0.5 for full contracts.
>
...
```
/wf design dark-mode-toggle-settings colorize
```
