// renderers/instrument.mjs — observability signals, dark paths, and dashboards.
...
export function render(artifact, ctx) {
  if (!artifact.siblingYaml) {
    return renderSimple(artifact, ctx, {
      title: `Instrument · ${artifact.frontmatter?.['instrumentation-framework'] ?? artifact.frontmatter?.title ?? ''}`,
    });
  }
...
function renderInstrument(artifact, ctx) {
  ...
  const headerHtml = artifactHeader({
    crumb: artifact.path,
    h1: `Instrument · <code>${escapeHtml(sy.framework ?? fm['instrumentation-framework'] ?? fm.title ?? '')}</code>`,
    badges: [
  ...
  const darkHtml = darkPaths.length
  ...
  const dashboardsHtml = dashboards.length
    ? `<section class="aug-result aug-instrument-dashboards">
        <h2 class="sdlc-h2">dashboards</h2>
        <ul>${dashboards.map((dashboard) => `<li><code>${escapeHtml(dashboard.name ?? dashboard.title ?? dashboard)}</code>${dashboard.url ? ` · <span>${escapeHtml(dashboard.url)}</span>` : ''}</li>`).join('')}</ul>
       </section>`
    : '';
