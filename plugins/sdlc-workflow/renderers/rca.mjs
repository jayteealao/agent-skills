// renderers/rca.mjs — root-cause analysis with timeline, causal chain, and 5-whys.
...
function renderRca(artifact, ctx) {
  ...
  const causes = (sy.contributing_causes ?? [])
    .map((cause) => callout('warn', escapeHtml(cause.title), `<p>${escapeHtml(cause.body ?? '')}</p>`))
    .join('');
  const mitigations = (sy.mitigations ?? [])
    .map((mitigation) => callout(
      'info',
      `${escapeHtml(mitigation.at ?? '')} · ${escapeHtml(mitigation.title)}`,
