// renderers/experiment.mjs — experiment hypothesis, variants, ramp, and guardrails.

import { md2html } from './_markdown.mjs';
import { artifactHeader, statusBadge, stageBadge } from './_shell.mjs';
...
export function render(artifact, ctx) {
  if (!artifact.siblingYaml) {
    return renderSimple(artifact, ctx, {
      title: `Experiment · ${artifact.frontmatter?.['flag-name'] ?? artifact.frontmatter?.title ?? ''}`,
    });
  }
...
function renderExperiment(artifact, ctx) {
  ...
  const headerHtml = artifactHeader({
    crumb: artifact.path,
    h1: `Experiment · <code>${escapeHtml(flagName)}</code>`,
    badges: [
  ...
  const hypothesisHtml = sy.hypothesis
    ? callout('info', 'hypothesis', `<p>${escapeHtml(sy.hypothesis)}</p>`)
    : '';

  const armsFigure = arms.length
    ? figureCanvas({
        figureNumber: 1,
  ...
  const armCards = arms.length
    ? `<section class="aug-result aug-experiment-arms">
        ...
        <dl class="exp-arms">${arms.map((arm) => {
          ...
          return `<dt><code>${escapeHtml(id)}</code> · ${escapeHtml(pct)}%</dt>
  ...
  const rampHtml = ramp.length
    ? `<section class="aug-result aug-experiment-ramp">
