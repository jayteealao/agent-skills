// renderers/sync-report.mjs — branch-sync health report (regenerable).
//
// A view over git/PR state: how far the feature branch has drifted from its
// base and whether a rebase is safe. When the run ships a sibling 00-sync.yaml
// the renderer builds a verdict, a diverging-bar figure (commits ahead/behind
// the base node), a diverged-files table, and a rebase recommendation. Falls
// back to the simple renderer when sibling YAML is absent (mirrors
// benchmark.mjs / profile.mjs).
//
// Dual-DOM (M-S4): the 920-unit diverging-bar SVG and the wide drift table are
// unreadable at 390px, so phones get a 2×2 metric-tile grid + a directory-
// grouped file list instead. The verdict and the recommendation callout are
// shared — they restyle on mobile via the reused `.verdict` / `.callout` names.
//
// SVG colours are literal hex matching the design tokens (the established
// convention in dashboard.mjs / plan.mjs); CSS `var()` is not honoured inside
// SVG presentation attributes, so baking the hex keeps the figure deterministic.
...
import { callout, severityChip } from './_icons.mjs';
import { escapeHtml } from './_validator.mjs';
import { renderSimple } from './_simple.mjs';

// conflict_risk → verdict tone. Production verdict classes are ship/caveats/no
// (the design kit's ok/warn/bad); the glyph (✓ / ◐ / ✗) is injected by CSS off
// `.verdict-X`, so the markup stays semantic.
const RISK_VERDICT = {
...
const RISK_WORD = { none: 'No', low: 'Low', med: 'Medium', high: 'High' };

const PX_PER_COMMIT = 25;
...
export function render(artifact, ctx) {
  if (!artifact.siblingYaml) {
    return renderSimple(artifact, ctx, {
      title: artifact.frontmatter?.title ?? `Sync · ${artifact.frontmatter?.branch ?? artifact.frontmatter?.slug ?? ''}`,
    });
  }
...
function renderSync(artifact) {
  ...
  const headerHtml = artifactHeader({
    crumb: artifact.path,
    h1: `Branch sync · <code>${escapeHtml(branch)}</code>`,
  ...
  // Verdict (shared across breakpoints — restyles on mobile).
  const verdictHtml = verdictBlock(verdict.kind, verdict.label, verdictSummary(sy, { ahead, behind, base, stale, conflicts }));

  // Desktop metric-row ↔ mobile metric tiles.
  const metricsHtml = metricRow([
    { label: 'ahead', value: ahead, ann: `commits not in ${base}` },
    { label: 'behind', value: behind, tone: behind ? 'warn' : undefined, ann: 'upstream commits missing' },
    stale != null && { label: 'stale', value: stale, ann: 'days since last rebase' },
    { label: 'conflicts', value: conflicts, tone: conflicts ? 'bad' : undefined, ann: 'files with dual edits' },
  ].filter(Boolean));
  const tilesHtml = syncTiles({ ahead, behind, stale, conflicts });

  // Desktop diverging figure + drift table ↔ mobile grouped file list.
  ...
  const desktopBlock = `<div class="d-only">${metricsHtml}<h2 class="sec">Divergence from base</h2>${figureHtml}${
    files.length ? `<h2 class="sec">Diverged files</h2>${tableHtml}` : ''
  }</div>`;
  const mobileBlock = `<div class="m-only">${tilesHtml}${
    files.length ? `<div class="subhead">Diverged files</div>${filesMobile}` : ''
  }</div>`;

  const recHtml = sy.recommendation
    ? callout(risk === 'high' ? 'risk' : 'warn', 'Recommended next step', `<p>${escapeHtml(sy.recommendation)}</p>`)
    : '';
...
// Local verdict block: same shape as _icons.verdictBlock but the eyebrow is
// title-cased ("Verdict") to match the design hand-off, and the summary is
// passed through escapeHtml here so callers stay HTML-free.
...
function verdictSummary(sy, { ahead, behind, base, stale, conflicts }) {
  ...
  return bits.length
    ? `${bits.join(' · ')}.`
    : `${ahead} commit${ahead === 1 ? '' : 's'} ahead of ${base}; nothing to reconcile.`;
}

/* ── mobile tiles ───────────────────────────────────────────────────────── */

function syncTiles({ ahead, behind, stale, conflicts }) {
  const tile = (lbl, val, ann, blocker) =>
    `<div class="mtile"><div class="lbl">${escapeHtml(lbl)}</div><div class="val${blocker ? ' is-blocker' : ''}">${escapeHtml(String(val))}</div>${ann ? `<div class="ann">${escapeHtml(ann)}</div>` : ''}</div>`;
...
function divergingBarSvg({ ahead, behind, base, branch }) {
  ...
  const baseNode = `<line x1="${BASE_X}" y1="68" x2="${BASE_X}" y2="155" stroke="${HEX.ink}" stroke-width="1.5"/>
    ...
    <text x="${BASE_X}" y="168" text-anchor="middle" font-size="11" font-weight="600" fill="${HEX.ink3}" letter-spacing="0.5">base · ${escapeHtml(base)}</text>`;

  const behindBar = behind
    ? `<rect x="${behindX.toFixed(0)}" y="94" width="${behindW.toFixed(0)}" height="32" rx="3" fill="${HEX.medBg}" stroke="${HEX.med}" stroke-width="1"/>
       ...
       <text x="${(behindX - 8).toFixed(0)}" y="101" text-anchor="end" font-size="11" fill="${HEX.med}">${escapeHtml(base)} has moved on</text>`
    : `<text x="${(behindRight - 8).toFixed(0)}" y="${CY + 4}" text-anchor="end" font-size="11" fill="${HEX.ink3}">0 behind</text>`;

  const aheadBar = ahead
    ? `<rect x="${aheadX}" y="94" width="${aheadW.toFixed(0)}" height="32" rx="3" fill="${HEX.accentSoft}" stroke="${HEX.accent}" stroke-width="1"/>
       ...
       <text x="${(aheadX + aheadW + 8).toFixed(0)}" y="101" text-anchor="start" font-size="11" fill="${HEX.accent}">${escapeHtml(branch || 'this branch')}</text>`
    : `<text x="${aheadX + 8}" y="${CY + 4}" text-anchor="start" font-size="11" fill="${HEX.ink3}">0 ahead</text>`;
...
// Evenly-spaced commit dots inside one arm, capped so a clamped arm stays tidy.
...
// Split a "+42/−11" display string into coloured add/del spans; pass anything
// unparseable through escaped, render an absent delta as an em-dash.
