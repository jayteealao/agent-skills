// renderers/benchmark.mjs — benchmark augmentation metrics and comparisons.
...
export function render(artifact, ctx) {
  if (!artifact.siblingYaml) {
    return renderSimple(artifact, ctx, {
      title: `Benchmark · ${artifact.frontmatter?.title ?? artifact.frontmatter?.mode ?? ''}`,
    });
  }
...
function renderBenchmark(artifact, ctx) {
  ...
  const sourceHtml = source
    ? `<p class="sdlc-crumb">source <code>${escapeHtml(source)}</code></p>`
    : '';
  ...
  const commandsHtml = commands.length
    ? `<section class="aug-result aug-benchmark-commands">
...
function benchmarkRow(metric) {
  ...
  const delta = metric.delta_pct != null
    ? Number(metric.delta_pct)
    : (before == null || before === 0 || after == null ? 0 : ((after - before) / before) * 100);
  const isLower = metric.direction !== 'higher-is-better';
  const improved = metric.verdict
    ? ['pass', 'improved', 'ok'].includes(String(metric.verdict))
    : (isLower ? delta < 0 : delta > 0);
