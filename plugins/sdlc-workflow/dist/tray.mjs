...
// scripts/tray.mjs
import { existsSync as existsSync3, readFileSync as readFileSync2, mkdirSync as mkdirSync2, copyFileSync, statSync, chmodSync } from "node:fs";
...
function autostartLauncherDir({ platform = process.platform, env = process.env, home = homedir() } = {}) {
  if (platform === "win32") {
    const appData = env.APPDATA || join2(home, "AppData", "Roaming");
    return join2(appData, "Microsoft", "Windows", "Start Menu", "Programs", "Startup");
  }
  if (platform === "darwin") return join2(home, "Library", "LaunchAgents");
  const xdg = env.XDG_CONFIG_HOME || join2(home, ".config");
  return join2(xdg, "autostart");
}
...
function autostartLauncherPath({ platform = process.platform, env = process.env, home = homedir(), startupDir } = {}) {
  ...
  return join2(dir, autostartLauncherName(platform));
}
...
function enableAutostart({
  ...
  const dir = startupDir ?? autostartLauncherDir({ platform, env, home });
  const path = join2(dir, autostartLauncherName(platform));
...
function iconBase64(state) {
  ...
  try {
    return readFileSync2(resolve(PLUGIN_ROOT, "assets", `app-icon${suffix}.${ext}`)).toString("base64");
  } catch {
    return "";
  }
}
function ensureRuntimeBinary() {
  ...
  if (!existsSync3(vendored)) throw new Error(`tray: vendored helper missing at ${vendored}`);
  const dir = join3(sdlcHomeDir(), "bin");
  mkdirSync2(dir, { recursive: true });
  const runtime = join3(dir, name);
  let stale = true;
  try {
...
async function selfcheck() {
  ...
  const vendored = resolve(PLUGIN_ROOT, "bin", "tray", name);
  const hasBin = existsSync3(vendored);
...
async function main() {
  ...
  if (isAutostartEnabled()) {
    try {
      refreshAutostart({ nodePath: process.execPath, trayBundle: TRAY_BUNDLE });
    } catch {
    }
