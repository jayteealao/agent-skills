...
// lib/hub-config.mjs
import { existsSync as existsSync2, mkdirSync, readFileSync as readFileSync2, writeFileSync, renameSync, rmSync } from "node:fs";
import { dirname, join as join2 } from "node:path";
var HUB_CONFIG_VERSION = 1;
var HUB_DEFAULT_PORT = 48173;
var LEGACY_DEFAULT_PORT = 4173;
...
function migrateHubConfig(raw) {
  const config = migrate(raw);
  const changes = [];
  if (Number(config.port) === LEGACY_DEFAULT_PORT && config.portMigratedFrom === void 0) {
  ...
  try {
    ...
    if (changes.length) {
      ...
      for (const c of changes) {
        try {
          logLifecycle({ event: `${c.key}-migrated`, reason: `${c.key} ${c.from} \u2192 ${c.to} (hub-config.json rewritten)` });
        } catch {
        }
      }
    }
