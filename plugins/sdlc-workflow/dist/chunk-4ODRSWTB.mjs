...
function countPart(value, singular, plural = `${singular}s`) {
  if (value == null || value === "") return null;
  const n = Number(value);
  return `${escapeHtml(String(value))} ${n === 1 ? singular : plural}`;
}
function blockerPart(count, label = "blocker") {
...
function sliceCard({ slug, fm = {}, meta = [], href }) {
  ...
  return `<a class="slice-card ${st}" href="${escapeHtml(href ?? pageHref(slug))}">
    ...
    ${parts.length ? `<div class="sc-meta">${parts.join(" \xB7 ")}</div>` : ""}
...
function sliceGridFigure(slices, markerId = "slice-arrow") {
  ...
  for (const s of slices) computeDepth(s.slug, /* @__PURE__ */ new Set());
