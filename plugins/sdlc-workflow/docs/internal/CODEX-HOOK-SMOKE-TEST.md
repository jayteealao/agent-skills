...
bump (`HOOK_CONTRACT_VERSION` in `hooks/_adapter.mjs`), or a `hooks/codex.hooks.json` change.
...
- [ ] **Repo harness green.** From `plugins/sdlc-workflow/` (one tree serves both hosts; the Codex wiring is `hooks/codex.hooks.json`):
  ```
  npm test                     # host-neutrality gate + runtime-parity + codex-hooks + the rest of the suite
...
`verify-deployment` — a `buildId`/version mismatch means the installed snapshot is behind; `codex plugin marketplace upgrade`
before retrying).
