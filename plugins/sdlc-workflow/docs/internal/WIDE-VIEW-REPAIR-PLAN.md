...
Build note (2026-09-07) — W11.10 built with one finding the plan did not
predict. The hub already rendered only queued views at start; what it did not
do was consult the version gate until the first 10 s reconcile tick, so a
stale view waited a tick and a fresh view was never rendered at start. The
change moves the heal pass to start and adds the counts line; the "3 fresh
repos render 0" gate holds as a unit test with an injected render spawn, not
an e2e run (the e2e acceptance runner has no hub fixture). The heal-on-
reconcile test in `multi-repo-hub.test.mjs` stamped its drift before the
server started and so was healed by the new start pass before its SSE client
connected; it is re-pointed to stamp the drift after the client connects, and
the feat commit landed red for one run because the commit script's gate read
the guard suite's `# fail 0`; the fix commit and the script repair are
...
Build note (2026-09-08) — fresh-eyes review corrections, one commit after
`fc2058d4`. The release sequence in RELEASE-DISCIPLINE.md could not end
green: `npm version` writes a `vX.Y.Z` subject the release guard ignored,
step 5 repeated the commit step 1 had made, `npm version` refuses the dirty
tree the other sessions' files leave, and step 7 failed until the hosts were
reinstalled. Fixed with `.npmrc` (`message=release(sdlc-workflow): v%s`), a
wider `RELEASE_SUBJECT`, and a rewritten sequence (gates → CHANGELOG commit →
`npm version` → push → reinstall → `verify:release`). The Codex
`session-start` adapter ran `hub-ensure --bootstrap` in every directory, so
the W11.3 litter came back from that host; the adapter now applies the same
policy (`insideGitCheckout` in `_adapter.mjs`), and hub-ensure gates the
record on `.ai/workflows`. `doctor` reported ACTION on a one-host machine
(`absent` was a gap) and had no row for the code browser or the W11.9
deprecations, which is where the "hub-ensure's stderr" note above lands: the
user-visible surface is the doctor row. The port migration left the old hub
on 4173 running (the recover path only dropped the record); the supervisor
now reaps a live hub on the recorded port before the start. Smaller: a
read-only `readHubConfig` no longer rewrites the file, the Codex PostToolUse
bound sits under the host timeout, `appendCostRow` appends instead of
rewriting, the catch-up count no longer double-subtracts, two dead imports
...
| W7 | One version carrier + render gate on renderer bytes | built | `7e755fbf` | `scripts/stamp-version.mjs` + the package.json `version` lifecycle script (`npm version <level>` stamps 5 carriers, builds, verifies, stages by path; review 2026-09-08: `.npmrc` release subject + wider `RELEASE_SUBJECT` + sequence rewritten); `_shell.mjs` literal removed (reads `runtimeVersion` from the manifest); `rendererBuildId` = sha256 over renderers/, view-src/, components/ decides render freshness before buildId and version; CSS/JS cache-buster is its 12-char prefix; `verify:versions` requires the manifest + a 64-hex `rendererBuildId` and fails on a shell literal; dist rebuilt; 7 sentence tests migrated + 5 new stamp tests; e2e 50 types green |
...
| W11.1 | `doctor` + installed check + cutover record | built | `3228b18f` | `lib/doctor.mjs` + `scripts/doctor.mjs` (`npm run doctor`, tray "Run doctor…", bundled to `dist/`); `verify-release-pushed.mjs` `installed` check (blocking off CI, advisory on CI, `--skip-installed`); SINGLE-SOURCE-CUTOVER.md §2 steps 0 + 7 and §5 before-record; 9 tests. Review 2026-09-08: `absent` host is not a gap, `disabled` verdict, code-browser + deprecated-keys rows |
