...
<p>Fires before a Write to <code>*/.ai/workflows/&lt;slug&gt;/*.md</code>. Blocks writes that violate:</p>
<ul>
  <li><strong>Filename convention</strong> — <code>NN-stagename.md</code> / <code>NNa-stagename.md</code> (substages like <code>02b-design.md</code>), plus the allowlist <code>risk-register.md</code>, <code>estimate.md</code>, <code>announce.md</code>, <code>po-answers.md</code>, and anything under <code>design-notes/</code>.</li>
  <li><strong>YAML frontmatter</strong> — must exist between <code>---</code> markers with <code>schema</code>, <code>type</code>, <code>slug</code> (the <code>po-answers.md</code> prose log is exempt).</li>
...
<p>Note: <code>.ai/workflows/INDEX.md</code> (the registry) and <code>.ai/ship-plan.md</code> (project-level) are handled separately — the per-slug path pattern doesn't match them.</p>

<h2>post-write-verify (PostToolUse: Write/Edit/MultiEdit/NotebookEdit)</h2>
...
<h2>post-write-render (PostToolUse)</h2>
