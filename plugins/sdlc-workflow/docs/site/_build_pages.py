...
<a class="brand" href="{base}index.html">sdlc-workflow<small>plugin docs · v9.9.0</small></a>
...
<div class="summary">
<table>
<tr><th>Pre-conditions</th><td>Plugin v9.5.0+. A repo open. No existing <code>.ai/ship-plan.md</code> — if one exists, see <a href="../reference/commands.html">amend ship-plan</a> instead.</td></tr>
<tr><th>Goal</th><td>Write <code>.ai/ship-plan.md</code> with all seven blocks (A–G) populated.</td></tr>
...
<p>Six templates ship with the plugin. Pick the one that best matches how you release:</p>

<table>
<thead><tr><th>Template</th><th>When</th></tr></thead>
<tbody>
<tr><td><code>kotlin-maven-central</code></td><td>JVM library published to Maven Central via Sonatype + GPG signing.</td></tr>
<tr><td><code>npm-public</code></td><td>npm package published to the public registry, often with provenance.</td></tr>
<tr><td><code>pypi</code></td><td>Python package via Trusted Publisher (OIDC) or API token.</td></tr>
<tr><td><code>container-image</code></td><td>Container image to GHCR/ECR/Docker Hub (multi-arch).</td></tr>
<tr><td><code>server-deploy</code></td><td>Kubernetes / ECS / fly.io rolling deploy.</td></tr>
<tr><td><code>library-internal</code></td><td>Internal/private registry; merge-on-main = released.</td></tr>
</tbody>
</table>

<h2>Step 2 — Run init</h2>

<pre><code>/wf-meta init-ship-plan --from-template &lt;kind&gt;</code></pre>

<p>Or omit the flag to be asked. Claude reads the template, then asks 13 grouped questions covering blocks A–G.</p>

<h2>Step 3 — Answer the block questions</h2>

<p>The plan has seven blocks:</p>
...
<dd>Semver/calver/sequential. Which files hold the version literal. Bump rule (git-cliff, conventional commits, manual). Prerelease/post-release suffixes.</dd>

<dt><strong>C — CI/CD contract</strong></dt>
<dd>Release trigger (tag-on-main, merge-to-main, manual-dispatch). Release workflow file. Required secrets. Publish dry-run + publish commands.</dd>
...
<dd>Default rollout (immediate/staged/canary/flag). Rollback mechanism. DB migration reversibility.</dd>

<dt><strong>F — Recovery playbooks</strong></dt>
<dd>Templates pre-seed two or three (signing failures, token 401s, version-already-uploaded). Add more over time as runs hit new failure modes.</dd>

<dt><strong>G — Announcements</strong></dt>
<dd>Channels (#releases, mailing list). Optional template path.</dd>
</dl>

<h2>Step 4 — Confirm + write</h2>

<p>Claude shows a summary table and asks for confirmation. <code>Confirm</code> writes <code>.ai/ship-plan.md</code> at repo root. <code>Adjust</code> goes back to one block. <code>Cancel</code> discards.</p>

<h2>Step 5 — Commit the plan</h2>
...
git commit -m "chore: author ship plan (template: npm-public)"</code></pre>

<h2>Amending the plan</h2>

<p>When CI/CD changes, secrets rotate, or you accumulate enough run experience to add recovery playbooks:</p>

<pre><code>/wf-meta amend ship-plan</code></pre>

<p>Pick which block (A–G) to edit. The amend flow re-runs only that block's questions. <code>plan-version</code> bumps by 1.</p>
