...
- **A second brainstorm `done` walks every unscoped decision.** A board distilled before 9.163.0 has no keep, cut, or later answers, and the rule walked only the decisions added since the last `done`, so it could skip all of them. It now walks every decision with no `scope` value.
...
- **The strip after the first live test** (MOD-FEATURES.md §5). The workflow row wraps to the band's width instead of truncating, and carries a `⇄ N more` button that walks the other active workflows; `/wf-active [slug]` does the same from the prompt. The pinned status line no longer repeats the strip: it reads `next /wf verify alpha-flow auth · $0.42 stage · hub 9.157.0`, the three things the strip's identity row does not show, so it stays useful while the plugin panel is hidden. `scripts/mod-fixture.mjs <dir>` writes a throwaway repository (two workflows, a roster, a cost ledger, a review ledger pair, a driver journal, a ship-plan audit) for live tests away from real projects.
- **The picker and the strip after the second live test** (MOD-FEATURES.md §5). Rows carry digits only: a page holds nine rows, and the letter hotkeys are gone. In the filter field one digit then Enter picks that row of the page on screen and `0` then Enter turns the page; a bare digit no longer narrows the rows to nothing. A `← back` button returns to the step before. The rotate ring walks every workflow, active ones first and closed ones after, so a rotate can always come back round. The active workflow is kept in the plugin store per repository, so a `/config` change (which reloads the module) or a new session opens on the workflow last named; a fresh repository opens on the newest active workflow, and on a closed one only when none is active. The strip's dim row holds the cost figures only; the hub stays in the status line and under the logo. Reads of files that may be absent (`03-slice.md`, `cost.jsonl`, the driver journal, the ship-plan audit, an index's mtime) check existence first, so the engine's error log no longer fills with ENOENT lines. The fixture now carries full-schema artifacts (a confirmed `stack:` block, a shape with `augmentations-needed` and NFRs, one file per slice, plan/implement/verify/review for `auth`), so `/wf plan alpha-flow ui` and `/wf implement` pass their Step 0 gates; `--dead` ages the driver journal past the 20-minute floor.

## [9.157.0] - 2026-09-16

### Added

- **Eight session aids around the `/wf` picker, and a dashboard pane** (WF-MOD-UX-PLAN.md W1–W7, all in the mod `hooks/mod/register.ts` with the pure helpers in `hooks/mod/active.ts` and the views in `hooks/mod/strip.tsx`). A strip above the prompt, under the picker when it is up, shows the active workflow's stage, selected slice with the roster's complete count, and next invocation; the same text, shortened, is the plugin's pinned status line, and `wf:<stage>` joins the footer's mode labels. A dim cost row under it shows the last `/wf` turn in dollars (from `$.session.usage`) and the workflow's `cost.jsonl` in tokens. The active workflow is the last one a `/wf` run or a `/wf` prompt named, else the one whose `00-index.md` changed last; a `tool.call` hook on Write, Edit, and NotebookEdit refreshes it after every write under `.ai/workflows`. After a turn that wrote an artifact, the workflow's next invocation becomes the prompt box's dim suggestion (`$.prompt.suggest`; Tab takes it, Enter runs it), and the engine's own suggestion yields to it while a workflow is active. A `/wf <key> <slug> [slice]` turn that ends with an answer and neither wrote nor touched the stage's artifact toasts `wf: <key> ended without <file>`. During an intake or shape turn each AskUserQuestion dialog's question ends with `(question N, floor 20)`. From a `/wf auto` or `/wf yolo` turn on, the status line reads `.driver-journal.jsonl` every five seconds and shows the run, stage, agent, elapsed minutes, and last beat; the driver runs in the background past its own turn, so the watch outlives the turn, and at `presumed dead since <time>` (silence past the run's own longest gap, 20-minute floor) it toasts once and stops. A `turn.start` whose text is the expanded skill takes its command from the dispatcher run of the previous ten seconds. While a `/wf` turn runs the spinner names the stage (`Implementing auth…`). At session start one dim line under the logo reports the hub (`sdlc hub <version> · <n> repos · <m> renders stale · /wf-doctor`), polled once a minute with one toast per state change. `/wf-dashboard` opens a pane with every workflow, its roster marks, the open findings of each review ledger (the sweep-level sibling YAML, else the markdown's unchecked rows), the ship-plan audit's open BLOCKER and HIGH findings as its triage gate counts them, and the hub; its `status` button fills `/wf status <slug>` and its `pick` button opens the picker at the slice step. Every aid has a boolean switch in the manifest's `userConfig` (`strip`, `suggestNext`, `stageCheck`, `questionProgress`, `driverStatus`, `spinnerVerb`, `cost`, `hubNotice`), shown in `/config` and read at `config.set`. Thirteen kit tests and fourteen harness tests. Unverified live: the `userConfig` rows in `/config` and the pane's draw; probes P1–P6 of the picker plan are still open.
...
- **A `/wf` picker for Claude Code, as a mod (function hooks).** `hooks/hooks.json` now names a hooks module, `hooks/mod/register.ts`, beside the classic hooks. At `session.start` the module registers one command per key, `/wf-intake` through `/wf-observability`, each with the key's description and argument hint, so the native typeahead lists every key when you type `/wf`. At `command.run` of a bare `/wf`, of a `/wf <key>` that still needs a slug, or of any `/wf-<key>`, the module draws a list above the prompt: the keys, then the workflows under `.ai/workflows` (active first with their stage and selected slice, closed ones marked), then the slices of the picked workflow (the `03-slice.md` roster status plus the furthest stage file present: planned, implemented, verified). Arrow keys move, Enter picks, Escape returns to the prompt. The last pick writes the full command into the prompt box with `$.prompt.fill`; you press Enter to run it, or edit it first. A `/wf <key> <slug> ...` typed in full runs as before. The module loads only where function hooks are on (`CLAUDE_CODE_ENABLE_FUNCTION_HOOKS=1`, Claude Code 2.1.269 or later); everywhere else `hooks.json` reads as before, and Codex never reads it. Ported from the `getArgumentCompletions` picker of pi-sdlc, with slices added. The pure state machine and the workflow readers are covered by `tests/unit/wf-picker-mod.test.mjs` (Node 22.6+, type stripping); the hooks module by the kit tests in `hooks/mod/tests/` (`npm run test:mod`, needs the Claude Code CLI). `npm run typecheck:mod` checks the module against the declarations `/plugin-types` writes to `.claude/types/` (gitignored).

### Fixed

- **The test suite no longer reaps the operator's live hub, and the run fails if a test leaves a hub behind.** The cold-start acceptance test in `tests/unit/lib/multi-repo-hub.test.mjs` spawns the real renderer under a sandbox home that had no `hub-config.json`; the renderer's bootstrap runs `ensureHubLifecycle`, which read a default config naming the operator's port (48173), judged the live hub untracked, reaped it, and spawned a sandbox hub in its place. The test also returned as soon as `.last-render` landed, while the renderer was still in its bootstrap tail, so that tail found a closed hub and a dead pid record and recovered a detached hub that outlived the suite. Now every sandbox home in that file carries a hub-config on a private port picked once per file; the cold-start test points the config and the pid record at its own in-process hub and tracks the render processes so it waits for them to exit before teardown (the supervisor then adopts, and the test asserts the pid record is untouched); a file-level `test.after` stops anything left on the private port. `tests/unit/lib/hub-upgrade.test.mjs` cleanup now also stops the pid that health reports and waits for the port to go quiet: `stopHub` trusts the sandbox pid record, which on Windows names the wscript launcher until the hub rebinds it, so a rolled-back hub could outlive the test and fail every later run on port 41987. New `tests/helpers/hub-processes.mjs`; `tests/run-all.mjs` records the owner of the operator's hub port (from the real `~/.sdlc/hub-config.json`, else 48173) and the set of `hub-serve.mjs` processes before the suite, and `tests/unit/state-dir-guard.test.mjs` fails the run when the port owner changed or a hub-serve process started during the run is still alive (netstat on Windows, lsof/ps elsewhere; skips when a listing is unavailable).
...
- **Narrower exposure defaults (W11.8).** Without the write token, `/__sdlc/health` and `/__sdlc/registry` return repository basenames and no `viewDir`; the token (`x-sdlc-token`, from `hub.pid`) unlocks full paths, so the tray and the supervisors see what they saw before. New key `codeBrowser.acknowledgedTailnet` (default `false`): while `tailscale.enabled` is `true` and the key is not, the supervisors spawn the daemons with the code browser disabled (`effectiveCodeBrowserConfig`), every `__code` request answers 404 with a one-line reason, and health carries `codeBrowser: { enabled, reason }`. `bin/tray/SHA256SUMS` is committed; `scripts/tray.mjs` verifies the vendored helper against it (`verifyTrayHelper` in `lib/tray-autostart.mjs`) before copying it to `~/.sdlc/bin` and refuses on a mismatch with the reason in the tray log. `bin/tray/README.md` records the helpers' provenance.
- **Bounded start-up catch-up and a 5 s Codex wait (W11.10).** On hub start a view renders only when it fails the version gate (the heal pass now runs at start, ahead of the first 10 s tick) or carries queued work; every other view is skipped, and one `catch-up:` line in `hub.log` reports the counts. The Codex SessionStart hub-confirm wait is 5 s (was 20 s); when the wait expires the hook returns `started-unconfirmed`, prints one line on stderr, records no activation, and exits 0. `SDLC_HUB_CONFIRM_TIMEOUT_MS` overrides the wait.
- **One asset bundle, served by the hub; the prune log rotates (W11.11).** The hub serves `assets/` at `/__sdlc/assets/<buildId>/<name>` (immutable caching; the segment is a cache key only; basenames only). The renderer references that route on every page (`hubAssetBase` in `renderers/_paths.mjs`, or `--asset-base`) and copies nothing into `.ai/_view/_assets/`; a `--clean` render drops a copy left by an earlier release. The change is in the renderer tree, so the render version gate re-renders every view on the next hub start. A page opened from the file system has no stylesheet from this release on. The deprecated per-repo daemon serves the same route for this release. `registry.prune.log` rotates at 1 MB with two generations, like `hub.log`.
- **Renderer snapshots and shared helpers (view review 2026-09-08).** The eight lifecycle renderers with the most traffic (intake, shape, plan, verify, review, handoff, ship, dashboard) have golden-file cases in `tests/unit/snapshots/_fixtures.mjs`; the dashboard reads its clock from `ctx.now` when a caller passes one, so its page is byte-stable under test and unchanged live. `escapeHtml` (`_validator`), `humanRelative` (`_cards`, now with an injectable clock), `normalizeVerdict` and `countBySeverity` (`_icons`) each have one body; the private copies in `_history`, `_icons`, `_link-graph`, `dashboard`, `index`, `hub-dashboard`, `design-audit`, `design-critique`, and `review-dimension` are gone. The shared `escapeHtml` renders a missing value as an empty string, as the private copies did, instead of the text `undefined`. Rendered output is unchanged: every existing golden still matches.

### Deprecated

- **The dead render and serve paths (W11.9, step 1 of 2).** `view.renderDispatch: "inline"` (`.ai/sdlc-config.json`), `perRepoServe: true`, and `liveReload: false` (`hub-config.json`) are deprecated in 9.154.0 and are removed in the next release; the keys still work in this release. A session that sets one logs one `deprecated-config` line in `~/.sdlc/lifecycle.log` (`lib/deprecations.mjs`, called by `hub-ensure --session-start`, which only the SessionStart spawn passes on every host). The next release deletes `lib/serve-lifecycle.mjs`, `scripts/render-sunflower-serve.mjs`, the three keys, their tests, and the docs rows.

### Fixed

- **Release procedure (review 2026-09-08).** `.npmrc` sets `message=release(sdlc-workflow): v%s`, so `npm version` writes the subject the release guard watches; `RELEASE_SUBJECT` also accepts a bare `vX.Y.Z` subject. `RELEASE-DISCIPLINE.md` now runs the gates before the bump, commits the CHANGELOG heading before `npm version` (which refuses a dirty tree), reinstalls the hosts before `verify:release`, and names `--skip-installed` for the check right after the push. Before this the documented sequence could not end green: the bump commit had a subject the guard ignored, step 5 repeated a commit step 1 had made, and step 7 failed until the hosts were reinstalled.
- **`doctor` verdicts and rows (review 2026-09-08).** A host that is not installed is `absent` and not a gap, so a one-host machine reads OK; a Codex cache at the shipped version with no `enabled = true` line is `disabled`. The report and table gain the code browser's effective state (from `/__sdlc/health`; `codeBrowser.acknowledgedTailnet` switches the browser off on a tailnet-exposed hub, and nothing else said so) and the deprecated `hub-config.json` keys in use (the W11.9 warning reached only `lifecycle.log` and hub-ensure's stderr). The dead `resolvePluginRootFrom` is gone.
- **A hub left on the previous port is reaped (review 2026-09-08).** When `hub.pid` names a live hub on a port other than the configured one (the 4173 → 48173 migration, or an operator edit), `ensureHubLifecycle` stops that hub, logs a `reap` line with reason `port change <old> → <new>`, and only then starts on the configured port. Before this the recover path dropped the record and the old hub lived on as an orphan. `readHubConfig({ create: false })` returns the migrated port without rewriting the file; the supervisor's read rewrites it.
- **Second review 2026-09-08.** The Codex session-start gate threw on every layout where `.git` sits above the workflow store (`insideGitCheckout` used `dirname` without importing it) and the adapter's `exit(0)` hid the throw; fixed, with a nested-layout test that asserts silence. A hub now removes `hub.pid` on shutdown only when the record is its own, so a reaped hub's late `server.close` cannot delete the next hub's record. The test suite's temp `SDLC_HOME` carries a `hub-config.json` with a private port, and the end-to-end runner gets its own state dir, so neither can touch the operator's hub or `~/.sdlc`. `SDLC_DISABLE_ENSURE_HUB` and `SDLC_DISABLE_HUB_ENSURE` are one switch, honoured under both spellings on both hosts and documented. `RELEASE-DISCIPLINE.md` names `npm version --force` for a tree another session left dirty instead of stashing that session's work.
- **Smaller corrections (review 2026-09-08).** The Codex PostToolUse adapter bounds the bundle at 12 s, under the host's 15 s hook timeout, so a slow bundle reports instead of being killed. `appendCostRow` appends one line per row (the read-modify-write-rename let two sessions on one slug drop a row). The hub's `catch-up:` line counts a view that is both stale and queued once. `render-sunflower.mjs` drops an unused import. The tray section of the-machinery.html names the doctor item.
...
- **SessionStart is conditional (W11.3).** `session-start-orient` enqueues a bootstrap render and spawns hub-ensure only when the project root has `.ai/workflows`, sits inside a git checkout, and the payload `source` is not `compact`; hub-ensure spawns on `startup` and `resume` only. The Codex `session-start` returns on `compact` and (review 2026-09-08) on a root with no `.ai/workflows` or outside a git checkout, before the hub confirm; `hub-ensure --bootstrap` skips the queue record for a root with no `.ai/workflows`. `lib/registry.mjs` `validateEntry` refuses roots under the OS temp dir, a Claude scratchpad, or `.claude/worktrees` (`ephemeralRootReason`; the test suite sets `SDLC_ALLOW_TEMP_ROOTS=1` in `tests/run-all.mjs`). The operator machine on 2026-09-07 holds eight `.ai/_view` directories in roots with no workflow store, with eight pending queue records between them: `Documents/dev` (2 records), `dev/agent-skills`, `dev/HomeLab-PDU-V1`, `dev/pi-unified`, `dev/cyberdeck`, `dev/Learn Electronics` (4 records), `dev/VIC3` (2 records), `Documents/rollover`. Deleting them is the operator's step (`rm -rf <root>/.ai/_view` for each); neither host's hook recreates them.
- **Runtime store GC on every start (W11.5).** After every confirmed hub start and every adopt, `ensureHubLifecycle` calls `gcRuntimes` and removes the materialized builds nothing protects. Protected: the active build, the build it replaced (`active-runtime.json` now records `previousBuildId`), the bundled build, the live hub's build, and every build on a protected `runtimeVersion` (the existing never-remove rule). A removal writes one `gc` line to `~/.sdlc/lifecycle.log`. The same-version rule means a developer machine whose builds all carry one `runtimeVersion` keeps them all.
- **The test suite is hermetic (W11.7).** `tests/run-all.mjs` sets `SDLC_HOME` to a fresh temp directory for the whole run when it is unset, fingerprints the real `~/.sdlc` first (registry roots and shards, prune-log size, `hub.pid`, runtime builds, active build), and runs `tests/unit/state-dir-guard.test.mjs` after every other file; a changed fingerprint fails the run. Before this, tests registered temp repositories in the operator's registry and appended prune lines to `registry.prune.log`.
...
- `reference/task.md` (both trees) — `/wf task <description>`: brief with observable ACs (`01-task.md`), a mandatory `blast-radius` classification (`repo-local`/`local-env`/`shared-env`/`external-party`/`irreversible`), a scaled authorization gate (the bottom three classes always stop for a human; `irreversible` requires a named confirmation echoing exactly what will happen plus per-step rollback lines), self-authored `05-implement.md`/`06-verify.md`, and honest `skipped` markers for the stages a task does not walk. An existing non-closed slug as the first token attaches the task as a compressed slice with its per-AC evidence inline.
- `reference/intake/audit.md` (both trees) — `/wf intake audit <concern> [paths…]`: two-wave surface resolution (four blind search modalities → union → user-confirmed scope gate), lens selection from review's existing rubrics with a stated reason per lens, adversarial refutation before any finding lands (3 distinct-lens refuters for BLOCKER/HIGH, refuted candidates preserved in `## Refuted`), the `needs-runtime-evidence` decidability boundary (exact experiment named, routed to `/wf probe`), and route-don't-fix termini per finding. Re-runs merge into the ledger; zero findings is a valid, legible result.
- `reference/_findings-ledger.md` (both trees) — the accumulating-ledger merge law (stable IDs, `surfaced-at` preservation, resolve-sweep, `runs:` append, triage persistence) extracted from `review/_stage.md` so `intake/audit.md` consumes the same ledger without restating it. Registered in the `shared-reference-drift` guard (fingerprint: "absence means cleared").
- EVIDENCE-SCHEMA-CONTRACT **§7** (contract revision, header updated): the task rungs — `attested` (a named party confirmed the outcome, with citation) and `asserted` (a claim with no read-back; presumptively fictional, cannot close an AC) — plus the `live` gloss: re-reading a real non-runtime system of record after acting IS live observation. `asserted` counts into `metric-acceptance-mock-rung`, so the shipped `mockEvidenceGate` blocks `result: pass` on the same code path that blocks a mock-evidenced code AC; the hook's operator message now names both new rungs.
- `_compressed-slice.md` (both trees): `task` and `audit` join the `<op>` enumeration, and a new **Inline records** section states the rule both callers rely on — a compressed slice carries its records inline and spawns no children (a slice-mode audit is one-shot and does not accumulate).
- Consult objective triggers on both surfaces (registered in `consult-trigger-coverage`): task auto-consults at the gate on `shared-env`/`external-party`/`irreversible`, credentials/billing/prod data, or a missing rollback; audit auto-consults on zero findings over a >20-file surface, any BLOCKER, any `needs-runtime-evidence`, or a security/auth/migration/billing surface.
- Tests: `work-without-a-home.test.mjs` (roster drift guards across both trees — dispatch tables, keyword sets, the suggest-and-confirm row, the blast-radius gate, driver refusals, review-stage arms); hooks tests for the `asserted` gate message and the `workflow-type: task`/`audit` schema round-trips; the two `resolveViewPath` assertions in their two **different** sunflower tests (`01-task` → `intake/`, `01-audit` → `audit/`).

### Changed

- `SKILL.md` (both trees): 22 keys (Codex 21), a new **Minimal lifecycle** dispatch section, `task` in the not-a-known-key roster and the Step 0.5 self-resolving list, `audit` in the intake row's mode list.
- `intake.md` (both trees): twelve mode keywords, the `audit` row in the mode→span and mode→file tables, the suggest-and-confirm row ("defect hunt across a named subsystem with no specific symptom" → propose `audit`), and three new disambiguation bullets (`audit` vs `rca`/`discover`/`investigate`) — the misrouting that motivated the plan.
- `review/_stage.md` (both trees): the lifted merge-law bodies replaced with citations to `_findings-ledger.md`, and two new Step-0.5 prerequisite arms so `/wf review <task-slug>` and `/wf review <audit-slug>` stop cleanly instead of dying on "an implement record must exist".
- Drivers: `auto.md` (both trees) gains pause-and-route arms for `workflow-type: task` and `audit`; `yolo.md` grows to five classes — `audit` joins the terminal-analysis refusal (each accepted finding seeds its own workflow), `task` becomes a second genuine refusal (the authorization gate is a human gate no written policy may resolve).
- `verify.md` (both trees) places the two task rungs on the existing ladder; `status.md`'s `workflow-type` vocabulary reconciled against the schema enum (which gains `task` and `audit`).
- `lib/leak-lexicon.mjs`: `task` joins `WF_KEYS`, and two pre-existing drifts are fixed — `observability` was missing from `WF_KEYS` (the comment still said "20 live /wf keys"), and `STAGE_NAMES` omitted `discover`/`ideate`/`adopt`/`probe`/`simplify`/`update-deps`.
- `renderers/_paths.mjs`: `'01-task' → intake/` beside the change-mode leads; `'01-audit' → audit/` in the terminal-analysis block. Two lines, two different blocks, deliberately.
- Doc site: `commands` (22 keys + the `/wf task` entry), `intake-modes` (the canonical `audit` entry), `artifacts`, `choose-your-entry` (the "work is not a code change" and four-analysis-modes decision content), `investigation` (audit alongside rca/discover/investigate), `autonomous-drivers` (both refusals), plus key-count fixes on `review-dimensions` and `installation`.
- `tests/wf-fixtures.json`: audit + task routing fixtures added, and the description now states plainly that no test consumes the file — the fixtures are documentation, not protection.

## [9.149.0] - 2026-07-30

The 2026-07-30 intake-modes audit found that five of the eleven non-default intake modes could not complete on their own documented happy path — every dead-end a cross-file contract violation, because modes were validated as artifact authors but never traced as callers into their successors' preconditions. This release repairs all five dead-ends, generalizes the v9.145.0 decision lifecycle (closure, provenance, escalation ladders) family-wide, and adds the missing test class so the failure mode cannot ship silently again.

One behavioral change to know about: `discover`, `ideate`, and `rca` workflows now **close** — discover at write time (a verdict leaves nothing to pick), ideate and rca when the pick/route is recorded. `/wf status`'s Active list shrinks by design; the closed artifacts remain readable and forwardable via `from <slug>`.

### Added

- `tests/unit/skills/intake-terminus-contracts.test.mjs` — the caller-contract guard (written red-first against the pre-repair files): for every intake mode it asserts the terminus against the successor's preconditions — schema-legal enums in every fenced template, the full index required set (`stack`/`review-scope-confirmed`/`appetite`), object-form `progress`, valid dispatcher grammar, pipeline completeness through handoff/retro, closure sections, provenance citations, shared-tail citation, and `next-command`/`next-invocation` coherence. Both trees.
- `intake/_intake-provenance.md` (both trees) — `_investigate-provenance.md` renamed and widened into the family evidence-forwarding contract: one Detect section (explicit `from <slug>` wins; inferred exact-label matching for the two labeled sources, investigate and ideate), per-source Consume rows keyed by `workflow-type` (investigate, rca, discover, ideate, update-deps prior run, escalated change-mode, simplify findings), and a uniform link-back (`origin-<type>`, `superseded-by`, implicit pick/route).
- `intake/_change-mode-tail.md` (both trees) — the shared tail for fix/hotfix/refactor/update-deps: the per-mode value table, the full collision check, the D4 stack policy (fix/refactor confirm with one question; hotfix/update-deps/adopt write it unconfirmed and ride verify's caveat path), the ONE index template with the complete default-intake required set, the family tripwire-breach mechanism (recorded in the plan and mirrored as `intent-risks` entries — hard caps become recorded breaches), the gate rules (decision recorded on every branch; Adjust re-gates; Escalate closes the slug `close-reason: superseded` and forwards `from <slug>`), and the shared workflow rules. The four modes cite it and their drifted copies are deleted.
- `siblingYamlSchemas.rca-diagnosis` + hook selection — a pre-fix RCA (`status: ready-for-fix-routing`) validates its sibling YAML against a diagnosis-shaped variant (required: incident, title, started_at, chain, timeline; the resolution set is optional), selected by `post-write-verify.mjs` from the artifact's status, never by the author. A pre-fix diagnosis no longer has to fabricate a resolution timeline to get past the write hook. Shared sub-shapes moved to `$defs`.
- Decision closure family-wide: rca gains `# Route — decision closure` (`rca <slug> <plan|fix|hotfix|human-triage>` — `plan` continues the same slug, `fix`/`hotfix` close and forward `from <slug>`); ideate gains `# Pick — decision closure` (`ideate <slug> <idea-id>`) plus persisted Step-5 selections (`selected:` frontmatter); discover closes at write time (`close-reason: verdict-recorded`) with `recommended-routes:` and a new `## 0. What this decides` section recording the decision that rode on the verdict.
- Escalation ladders (probe with its real grammar `/wf probe <slug> "<question>"`, study-sources, consult) in rca, hotfix (confidence floor — a still-low root cause after the third agent stops into the ladder instead of shipping a guess), discover (executable rungs + effort escalation when a large decision rides on the verdict), refactor (fix-loop stall rungs; "Add tests first" now names its mechanism — a characterization-test plan step), and ideate (high-impact speculative candidates route to a named cheap check instead of a silent cull). Human triage is the last rung everywhere, never the only one. `probe.md` handles terminal-analysis slugs (no `03-slice.md`) without erroring.
- The index schema declares the `revisions:` ledger and the slug overview renders it — amend's revision entries are now schema-backed and visible.

### Changed

- **adopt** is now runnable: the schema-illegal `slices[].status: implemented` becomes `complete` (adoption carried by `provenance: adopted` + `progress`), and a new Step 0 adds slug derivation, the collision check, a same-branch active-workflow scan (stops into extension instead of forking another workflow's branch), provenance, and the stack fingerprint. The adoptable surface is one defined union (committed-ahead + staged + unstaged + enumerated untracked, minus `.ai/**` and `.scratch/**`); the merged-upstream refusal is decidable (`git fetch` + `git branch -r --contains` per ahead-SHA); W2d proxy+deferral resolutions land as real `runtime-evidence-deferrals` entries; `05-implement.md` gains `## Verification Seams Built`.
- **rca** routes are dispatcher-valid (`… from <slug>` forms — the old `intake fix <slug>` grammar dead-ended in a collision warning), its index carries `title`/`updated-at`/object `progress`/a `stack:` block (the plan route STOPs without one) and drops the phantom `selected-slice`, the synthesized shape's `status: ready` becomes the schema-legal `complete`, the rich-YAML step moves before the chat handoff, and the stale `wf-how` pointer is gone.
- **hotfix** reaches its own exit: the pipeline includes `/wf handoff` and `/wf retro` (ship STOPs without a ready handoff verdict; retro's incident consult exists exactly for hotfixes), `01-hotfix.md` carries ≤2 acceptance criteria (verify and review read them from the lead), `## Impact` seeds two `intent-risks` entries so ship adjudicates incident severity, prod-branch detection works offline first (`git symbolic-ref refs/remotes/origin/HEAD`), and the unwritable `≤3 files` plan constraint became a recorded tripwire.
- **fix** carries `stack:` with a one-question confirm, the `--design` flag became the dispatcher-registered trailing `design` token, and the index template carries the `origin-<type>` key its own Step 0 mandates.
- **update-deps** self-reports Steps 7/8 to the index (progress, workflow-files, registry row — completed dependency work no longer reads as unimplemented forever), audit-only ends lawfully (`/wf close <slug> deferred`, Hold entries pushed to `open-questions` as revisit triggers, `mode:` mirrored onto the index), a bare existing run-slug resumes, prior-run provenance seeds research from the last run's Hold/Blocked lists, and the contradictory `next-command`/`next-invocation` pairs are reconciled.
- **extend** gains the entry guard (terminal-analysis and pre-slice parents route to their own next step instead of failing on a missing roster), Step 6 becomes the single authoritative index writer (absorbs Step 3b's writes; re-points `next-command`/`best-first-slice`; revives a complete/closed parent; updates the registry row), from-review-all-bugs falls back to the interview instead of stopping, `po-answers.md` is created if absent, and `from-probe`/`from-simplify` join the seed set.
- **amend**'s review-scope amendment also sets `review-scope-confirmed: true` (the next slice/plan ask no longer silently overwrites the PO), a closed workflow WARNs and confirms, and a trailing `dry-run` previews without writing. **modernize**'s era table gains the v9.136 row (`review-scope-confirmed`/`appetite`/`stack`), the registry-row write, deferral `needed-by`/`absorbed-by`, and a report-only decision-lifecycle row naming the pick/route command; a `schema-modernized-at` + `schema-absent-fields` stamp makes runs idempotent (the dispatcher's schema-era nag respects it); the cardinal rule now states the absent-`slices[].status` backfill mapping explicitly.
- **ideate** seeds its lenses from recorded history (retro action items, `.ai/solutions/`, deferred review findings, `sdlc-debt:` markers), keeps per-idea `file:line` evidence on the machine-readable roster (`evidence:`), respects the 4-option question ceiling (numbered chat list beyond it), and renders extension-shaped ideas as extensions. Its index stays `status: ready` + `next-command: user-picks` until the pick closes it.
- **investigate**/terminal index templates gain `title`/`updated-at` and object-form `progress` (the renderer silently drops the list form). discover's code-explanation pointers move from `/wf recap` to the deep-research skill. The dispatcher's contract table reflects every changed terminus, and its trailing-token conventions (`from <slug>`, `design`, `dry-run`) are documented.
- Doc site: intake-modes reference, quick-lanes, investigation, and choose-your-entry updated to the new termini, closure semantics, and forwarding grammar.
...
A Playster `/wf yolo` slug stacked **five slices** of deferred live evidence behind "host port 8080 is held by another process" — while the app's own debug source set hard-coded `const FIRESTORE_PORT = 8080`, so the emulator could never be moved to a free port no matter how many times the wall was re-probed. Every existing mechanism fired *correctly* and the outcome was still wrong: the probes were real and receipted (attempt-before-declare worked), the repeat-deferral tripwire fired, the ladder even switched the *backend* suite to port 8081 in the same slug. The gap was that every one of them classifies a wall by **symptom** (which probe failed) when the decision hinges on **ownership** (who can dissolve it). Symptom said "port squatter → wait"; ownership said "fix our own constant." This release forces the ownership question first and gives the driver a lawful way to negotiate an environment instead of only waiting on one.

- **R1 — wall-ownership triage (the load-bearing change).** The constraint-resolution ladder gains a MANDATORY first move ahead of climbing: *would a change to code in THIS repo dissolve this wall?* → `wall-ownership: code-owned | environment-negotiable | external`. A `code-owned` wall (hard-coded host/port/endpoint in a debug source set, a fixture uid production rules reject, a harness pinned to one env-var name) is unfinished verifiability work wearing an environmental costume, and **verify's deferral hatch is now unavailable to it** until the repo-change option is surfaced as a decision — scoped in-slice, scoped as a prerequisite slice/harness, or declined on the record. The verdict is stamped on the deferral (`interactive-verification-wall-ownership`, plus `wall-ownership:` in the `00-index.md` schema) and on each plan `constraint-resolution:` line. Plan's tripwire and verify's 3-deep stacking STOP both **re-classify rather than inherit** — a wall first called `external` under time pressure is exactly the kind that turns out `code-owned` on a second look.
- **R2 — clearing events name an actor, not a hope.** `cleared-by` must target a *provisionable* act ("after slice X lands the configurable-port change, run `/wf probe <slug>` against any free port"), never a passive wait. "Once host port 8080 frees", "when a device becomes available", "when the environment allows" are rejected outright: a deferral pinned to state nobody in the loop controls is indefinite by construction, and it reads as progress in `/wf status` while nothing can ever move it. New `clearing-event:` field in the deferral schema; force-scope option 2 (`proxy+deferral`) is now **unavailable to a `code-owned` wall**, whose clearing event would only ever be a change the team can already write.
- **R3 — cost the wall before choosing.** Once the repeat-deferral tripwire fires, plan MUST write `wall-cost: retire ≈ <effort> | carry = <N> deferred AC across <M> slice(s) riding "<clearing event>"`, and `harness-declined:` is lawful only after that line exists. Stacks like Playster's accumulate only because nobody was ever required to put a ten-line debug-config change and ten deferred criteria side by side.
- **R4 — the env-remediation rung.** The step the ladder always permitted but never named, now explicit between "rungs exhausted" and "defer", bounded by **authority, not effort**. Allowed unprompted: rebind a harness-owned service to a free port and record it (*never defer over a port the run itself binds*), boot the documented headless mode, start an existing AVD, run a provisioning script the repo ships, execute a plan-authorized install. Requires an explicit decision: killing a process the run did not start, mutating host/system configuration, editing product code to make evidence collectible, or introducing an unplanned tool. `yolo` climbs the rung before any deferral and HARD-STOPs on a `code-owned` wall with no scoped resolution — it may neither patch product code to unblock itself nor defer around its own constant. `probe` — the actor most deferrals name as their clearing event — remediates and re-triages before re-recording a wall it was sent to clear.
- New drift-guard suite `tests/unit/skills/wall-ownership.test.mjs` (10 tests) pins all four across both trees, including triage-before-probe ordering, the remediation allow/deny lists, and the codex `$wf` dialect. Files: `runtime-adapters.md`, `verify.md`, `plan.md`, `probe.md` (both trees, CRLF preserved where the codex mirror uses it) + `yolo.md` (Claude-only). Docs-only behavior change — no schema, hook, or runtime surface touched.

### Fixed — test discovery is automatic; a new test file can no longer go silently unrun

Wiring the guard above exposed that `scripts.test` was a hand-maintained list of ~40 explicit paths, so a test file was inert until someone remembered to append it — failing in the quietest possible way (the file exists, passes when run directly, reviews clean, never runs in CI). **Three drift-guard suites had never executed once**: `steering`, `intake-shape-hardening` (v9.136.0), and `consult-trigger-coverage` (v9.139.0 — cited in that release's own notes as the guard pinning its change). All three were green when finally run, so nothing had rotted, but for weeks those releases believed in guards that were decorative.

- The obvious fix does not work: **no single `node --test` form spans this package's Node range.** Glob arguments (`node --test "tests/**/*.test.mjs"`) landed in Node 21 and are rejected outright by Node 20 — which `engines.node >=20.0.0` declares and `.github/workflows/sdlc-build-freshness.yml` pins; conversely a directory argument (`node --test tests/`) walks correctly on Node 20 but is treated as an entry *module* on Node 22+ (`MODULE_NOT_FOUND`), and the bare CWD walk is too broad — it sweeps committed `dist/` bundles. Verified empirically against v20.20.2 and v22.15.0 rather than inferred.
- So discovery moves into `tests/run-all.mjs` (plain `fs`, identical on every supported Node): it collects `**/*.test.mjs` under `tests/`, forwards flags to `node --test`, accepts substring filters (`npm test -- skills`), exits non-zero when discovery finds nothing, and **refuses to run** anything matching a known non-test path — `tests/e2e/acceptance.mjs` (its own `test:e2e` entry point, which builds a synthetic tree and drives the real renderer), `tests/helpers/*` (processes spawned *by* tests), and the two imported snapshot helpers. Discovery was proven set-identical to the 42-path list it replaces before the swap, and the refusal guard was exercised with a deliberately mis-named probe file.
- The **codex mirror carried the same latent pattern** (a 2-file explicit list) and gets the same runner; its suite is unchanged at 23/23.
- New `tests/unit/test-discovery.test.mjs` keeps both trees from regressing to enumeration — and, never having been added to any list, is itself the end-to-end proof that discovery works. Suite: 687 tests / 685 pass / 0 fail / 2 skipped, up from 651 before the gap was found.
...
A five-project transcript audit of every recent `/wf handoff` + `/wf ship` run (Waypoint, Trails, Isometric, Playster, bot-backend — ~25 sessions, 2026-07-11→18) found the gates themselves fired correctly in every session; the failures lived in what fed them (mock-green evidence, stale environment claims), in what `ship-plan build` generated (broken-on-first-run CI, unreviewed release code), and in ceremony/schema friction (re-fired drift questions, unrepresentable states, false yolo HARD-STOPs). Full audit → plan → build in `docs/internal/HANDOFF-SHIP-HARDENING-PLAN.md` (W1–W6, all built here).

- **W1 — schema breathing room.** `post-publish-checks[].status` gains `skip` (md/yaml parity — a Trails ship had to *delete* an N/A check from frontmatter to get past the hook) with the rule "a check that did not run records `skip`, never `pass`"; `go-nogo` gains `pending` so a ship paused before the step-5 gate stops being forced into a false `no-go` (a Playster run took 5 hook blocks recording one legitimate pause); the run-index `notes` cap doubles to 160 (tripped blind in 3 projects); and the new `probe-evidence/` subtree carve-out (`isProbeEvidencePath` in hook-utils, both write hooks) gives probe evidence a sanctioned home instead of forcing `.md` evidence to smuggle through `.txt`.
- **W2 — yolo driver correctness.** New `acKey()` normalizes AC identity in `probeGaps`/`collectDeferrals`/`deferralPressure`, killing the false HARD-STOP where a bare `residual[]` copy ("AC2") failed to dedupe against its receipted terminal copy ("AC2 — three…") — the defect that burned a 79-minute Playster resume twice and was previously patched only in a session's plugin-*cache* copy. Slug-mode now skips `status: skipped` roster slices (a 6.7-hour Waypoint run died walking one) and recognizes PO-recorded `ship-override-authorization` on index deferrals: authorized walls are excluded from re-challenge, waived from probe-receipt escalation, and count verify done on resume — the driver *reads* an authorization, never grants one. POLICY.verify rule 5: environment claims are probe results (command + output, re-executed fresh) and provisioning must persist beyond subagent context. 8 new gate tests (35/35).
- **W3 — readiness truth + persistent acknowledgements.** Handoff's verdict now *consumes* the `mergeStateStatus`/`mergeable` it always fetched: `BLOCKED`/`CONFLICTING` can no longer compute `ready` (a Waypoint handoff certified `ready` on a merge-BLOCKED PR held by 3 unresolved threads), with a one-shot thread-triage re-check; ship Step 6.0 re-verifies the merge gate before attempting, 6.2 gains an explicit no-silent-fallback path for GitHub refusing the configured merge strategy, and the Step 6 idempotency read drops the nonexistent `merged` JSON field ship.md itself prescribed. New readiness drift signals: `version-already-released` (a Trails handoff froze a versionName four days after that tag shipped — only session memory caught it; ship Step 1.2 also tag-checks before confirming) and advisory `compliance-stale`. Acknowledgements persist in `.ai/ship-plan-acks.yaml` (Step R2.5): fingerprinted per finding, structural branch-lifetime signals acknowledgeable once per branch (Isometric answered the identical pair 3×), invalidated wholesale by a `plan-version` bump, with a `pending-amend` re-fire guard replacing the verbatim re-ask of an already-answered question (Playster's one drift cost 3 STOP rounds). Handoff also re-validates a recorded `branch-strategy: none` against the packaged diff before delivering a document where a PR was needed.
- **W4 — ship's clean-tree gate classifies instead of blanket-stopping.** Step 1.1 buckets dirty paths: memory-seed fence files (mechanically verified, one-keystroke commit), `.ai/` bookkeeping per the new `artifactTracking` policy (`tracked | ignored` in sdlc-config, asked once at `ship-plan init` Step 3.5 with the `.gitignore` block written on `ignored`; `/wf status` surfaces a one-time advisory when unset), `ship-plan build` provenance output (never silently commit-and-include), and unknown → STOP as before. This retires the ad-hoc mid-ship repo surgery that once un-tracked 473 paths during a release.
- **W5 — `ship-plan build` must not break the next release.** Two new design-contract clauses ("the next release must stay green"; "never pin a version literal the repo already declares") and four new Step 17 validations beyond syntax: version-literal consistency vs repo declarations (the pnpm double-version bug that broke all five of one project's generated workflows on first CI run), `needs:`/`uses:`/secrets graph integrity, the repo's own formatters over every written file, and a provisioning probe — any gate whose infrastructure can't be confirmed is scaffolded inert behind `vars.SDLC_GATE_*` with a `gates-to-activate:` ledger (one build wired `needs: staging-smoke` with no staging apps and blocked production deploys for days). New Step 18.5 routes outputs explicitly — review slice (recommended) / commit-now with `sdlc-unreviewed: true` trailer / recorded-uncommitted — closing the hole where 34 unreviewed workflow lines entered a production release with an external bot as their only reviewer.
- **W6 — evidence direction + environment-claim skepticism.** Shape's AC authoring gains the direction rule: gate/guard ACs split into prove-pass and prove-fail-closed halves; verify and probe refuse to clear a fail-closed AC with happy-path evidence (an "unhealthy revision caught" AC was once cleared by a perfectly healthy release). Verify's escape hatch gains provision-before-declare (run the repo's own provisioning script before declaring its wall; ephemeral subagent provisioning doesn't exist afterward) and subagent-untrusted environment claims (orchestrator re-executes the probe once — a stale relayed "no device / port held" once stood for days and, when dissolved, exposed 4 defects mocked verifies had missed). Ship Step 9 + 1.5: pass-by-skip records `skip` never `pass`, and git-cliff never regenerates a curated CHANGELOG without a `cliff.toml` (it once clobbered 199 curated lines).
- Both trees (Claude `/wf` + Codex `$wf` dialect). `hooks/`+`lib/`+`yolo.js` changes → `dist/` rebuilt and codex runtime re-synced in the same release; `tests/frontmatter.schema.json` + `schemas/sdlc-config.schema.json` extended (`skip`, `pending`, `notes` 160, `artifactTracking`). Gates: `npm test` 649/0/2 (35/35 yolo gates incl. 8 new); `npm run verify` 51 pages stamped v9.138.0; `verify:codex` parity; `build` + `sync:codex` run.
...
A structural re-read of `intake`/`shape` after the INTENT-FIDELITY program (v9.126–131) found the remaining defects cluster in three themes: intake was the unguarded input to all of shape's gates (a lazy intake could author zero RIMs and legally no-op the adjudication gate; the Charter Scenario triggered only on a *numbered* core loop — a formatting accident); shape self-graded its own adversarial pre-mortem; and `shape.md` had accumulated structural drift (its numbered flow started at 3 with no steps 1–2, "Always launch sub-agent 2" contradicted its own skip criteria, and the PO tooling question lived inside a sub-agent prompt that can't reach the PO). Full analysis + waves in `docs/internal/INTAKE-SHAPE-HARDENING-PLAN.md` (W1–W4, all built here). Three PO directives constrained the design: the **20-question floor stays** (the aim is an interview exhaustive of all ambiguity), **review-scope is still asked** (just at the stage that can answer it), and consult must be always-on (it already is — the stale gate is deleted).

- **W1 — shape.md renumbered + tooling relay.** One contiguous Step 0→10 sequence (research launch = Step 1, interview = Step 2, artifact write = Step 10); the sub-agent-2 skip criteria are stated exactly once; sub-agent 1's interactive-tooling block is reporting-only and the **orchestrator** relays the tooling question to the PO at Step 3, built from the agent's actual findings.
- **W2 — input integrity.** Intake's `## Risks if Misunderstood` / `## Charter` may never be silently absent: a Step 6a **misreading pass** ("name the 3 most likely misreads") is the RIM quality floor, and zero entries is legal only as the explicit `intent-risks: none-declared` / `charter: none-declared` declaration (schema now admits the string; a new warn-only `intakeLedgerLint` in `post-write-verify` surfaces the gap at write time, `hooks.intakeLedgerLint: false` to opt out). Shape's Step 9a gains a **backfill branch** — a standard-lifecycle slug with no ledger and no declaration is re-derived from the intake, never waved through. The Charter Scenario now keys to the *work* (a core loop numbered in intake **or derivable from its prose**; skipping requires `charter-scenario: "none — <reason>"` in frontmatter), and intake's template asks for the loop as numbered steps. `appetite` lands in `00-index.md` frontmatter (machine-read by shape's pre-mortem horizon, slice, and plan's consult trigger). Intake **ratifies the charter with the PO** in one question at birth (`po-ratified: true` on charter entries). Extension mode (`intake <slug> <scope>`) authors an **intent-risk & charter delta** for net-new scope — adjudicated in place by its own interview, since no shape run follows.
- **W3 — interview coverage & grounding.** The 20-question floor is now *accountable*: Step 2.1 authors an **`## Ambiguity Inventory`** (AMB-n items harvested from intake/po-answers/code-map/research), every question names the item it closes or confirms (assumption-confirmation questions are first-class closers; padding is redefined as "closes or confirms no inventory item"), and a Step 2.5 **coverage gate** makes it illegal to end the interview with an inventory item that is neither closed, extension-targeted, nor parked in Unknowns. A conditional **Round 3b (visual direction)** budgets the 4 design-brief questions (register / color strategy / references+anti-goals / state inventory) ON TOP of the floor for UI work. The **review-scope question moves from intake Batch A to `slice`** (asked with the roster known, recommendation informed by slice count; `plan` asks on the skip-to-plan path; `review-scope-confirmed` tracks it — asked exactly once per workflow), intake's Batch order flips to **substance first, process second**, and a conditional **Step 0.7 bounded Explore pass** grounds Batch B questions in the code (`## Affected Areas (preliminary)`), with handoff clauses so shape's sub-agents *verify and deepen* intake's map and freshness findings instead of re-deriving them.
- **W4 — independence & surfacing.** Intake `default`/`ideate`/`investigate` get **objective consult auto-triggers** (the v9.135 pattern; closes 3 of the weak-15). Shape's pre-mortem runs in a **blind sub-agent** whose inputs are `01-intake.md` + `po-answers.md` ONLY — it can't rationalize decisions it never saw; the orchestrator adjudicates with citations; the consult pre-mortem fires on objective triggers. The **stale `externalDispatch.enabled` gate on shape's consult dispatch is deleted** (consult has been ungated since 2026-07; the flag governs only imagery/uiproto egress and its shipped default stays `false`). Shape's chat return gains a required **`fidelity:` line** (honored/narrowed/dropped dispositions + RIM outcomes — the reframing now appears where the PO actually reads), and a `dropped` directive requires a **this-stage ratification question**, never just a citation of an earlier answer.
- Both trees (Claude `/wf` + Codex `$wf` dialect: chat-numbered-list questions, `_timestamp.md`/`_subagents.md` refs, codex's own augmentation-plan wording preserved through the renumber). New drift-guard suite `tests/unit/skills/intake-shape-hardening.test.mjs` (17 tests, both trees + schema round-trips + hook fixtures). Schema: `appetite`, `review-scope-confirmed`, `charter-scenario`, `none-declared` ledger forms, `po-ratified` on charter entries. Gates: `npm test` 636/0/2; `npm run verify` 51 pages stamped v9.136.0; `npm run verify:codex` parity OK (191 files, shared buildId `a8394bad6705`); `build` + `sync:codex` run.
...
The reconciled Step 3 of `docs/internal/INTENT-FIDELITY-HARDENING-PLAN.md` and `YOLO-EVIDENCE-INTEGRITY-PLAN.md`, governed by the frozen `docs/internal/EVIDENCE-SCHEMA-CONTRACT.md`. One combined `hooks`/`lib`/`dist` rebuild + `sync:codex` carries every lexicon and verify gate. Cures the waypoint failure where all-green evidence was invisibly mock-rung: `drainStream` matched invented stream-event names, the mocks emitted the same invented names, so 9/9 passed while every real generation returned `undefined`.

- **evidence-rung, the load-bearing field.** Every user-observable AC records the **highest** rung that produced its evidence: `live | headless | emulator-or-container | cited-mock | uncited-mock | static | n-a` (the cited/uncited split is the mock-honesty distinction — a mock with no recorded provenance is presumptively fictional). `verify.md` (both trees) authors it in the AC status table with a per-slice rollup; `00-index.md` gains an `evidence-quality:` slug rollup.
- **User-observable-mock = not met (hard gate).** A user-observable AC whose rung is `cited-mock`/`uncited-mock`/`static` is not met — climb the ladder or defer. The `verify` frontmatter carries `metric-acceptance-mock-rung` (count of such ACs), and a new **`post-write-verify` mock-evidence gate HARD-BLOCKS `result: pass` while it is > 0** (opt-out `hooks.mockEvidenceGate`). This generalizes the shipped integration-blindspot guard from "AC asserts a live integration" to *all* user-observable ACs.
- **Per-AC skip = missing evidence (YOLO F4).** A skipped gating spec (guard exit, `.skip`/`.todo`, missing env/secret, filtered out) is a missing-evidence event for the AC it gates — that AC cannot inherit the suite's green. `verify` records `skipped-gating-specs: [{spec, ac, precondition}]`; the AC-gate routes it to `blocked-runtime-evidence-missing`. (The waypoint OAuth SHIP: the one seeded-session E2E that gated the broken flow was skipped.)
- **Mock provenance + fixture-fidelity (W5.1/F5.3).** A mock emulating an external interface records `mock-provenance` ("from recollection" is illegal → `uncited-mock`); a cheap grep of the installed package for the mocked identifiers with zero hits caps the AC at `partial`; a `fixture-fidelity: checked` spot-check against the real contract (`.d.ts`/docs/one schema call) upgrades `uncited-mock` → `cited-mock`.
- **First-light + deferral stacking + mitigation wiring (W5.2/5.3/5.4).** `unproven-integrations: [{name, first-light: null}]` caps dependent ACs at `partial` until a live observation; a deferral `absorbed-by` a **third** slice is a stop, not an absorption; a shape-mandated mitigation needs an AC that exercises the wired path, not "the code exists".
- **Limitation-claim + suppression-debt lints (W3.2/W9.3).** New shared `lib/limitation-lexicon.mjs`: a `does not exist / not exposed / was removed` comment with no citation (`source:`/`node_modules/`/`repro:`/`issue:`/URL) within ±3 lines draws a warn (`hooks.limitationClaimLint`); a new `as any`/`@ts-ignore`/`eslint-disable` with no `sdlc-debt:` marker within ±2 lines draws a warn (`hooks.suppressionDebtLint`) so the existing debt lifecycle inherits it. `plan.md`/`implement.md`/`intake/update-deps.md` carry the prose rules (comments are hypotheses; recalled shapes never justify `as any` alone). Wired into `post-write-verify` (code-file branch + artifact branch); named-mechanism warn (`hooks.namedMechanismLint`) completes the W7 hook deferred from R1.
- **`needed-by`, deadlines (W9.2).** Prerequisites/deferrals gain a `needed-by: <slice>` due-date; status escalates when the consuming slice completes with the prerequisite unmet.
- New tests: `tests/unit/lib/limitation-lexicon.test.mjs` (11 cases — citation adjacency, comment scoping, mechanism ownership), a `post-write-verify` mock-gate behavior test (blocks at rung 1, passes at rung 0), and R2 schema round-trips + both-tree verify.md drift guards in `intent-fidelity.test.mjs`. Four new opt-out keys in `schemas/sdlc-config.schema.json`. Codex mirrored in native idiom; `verify-claudisms` clean.
...
Implements Phase 1 of `docs/internal/YOLO-EVIDENCE-INTEGRITY-PLAN.md` (fixes F1/F2/F3/F6), governed by the frozen `docs/internal/EVIDENCE-SCHEMA-CONTRACT.md`. An empirical audit of 29 real `/wf yolo` sessions across 7 projects found the pointer-based driver design sound but surfaced two damage-causing failure classes — both about *how evidence claims flow between stages*, not individual subagent carelessness. Phase 1 converts the two prose "laws" `verify.md` already carries into driver-enforced structure on the Claude-only `workflows/yolo.js` orchestrator. No `verify.md`/hook/schema changes here (those are the reconciled Step 3, later); this ships with a version bump alone (yolo.js is not bundled into `dist/` and is never mirrored to Codex).

- **F1 — `POLICY.verify` now *restates* the deferral law, not a bare-excuse paraphrase.** The old `DEFER-DON'T-CANCEL` paragraph told the subagent to defer when the environment "genuinely CANNOT produce that proof (no emulator/…/credential is unreachable here)" — near-verbatim the bare phrasing `verify.md` explicitly rejects, and the last word the subagent read. Rewritten to the four-rung law: climb the constraint-resolution ladder first; **attempt before declare** (a defer-reason is invalid without a literal capability-probe command + output tail run *this* round); **never inherit** a prior artifact/slice/run's deferral (re-probe it fresh); and a **skipped/guard-exited spec is not evidence** for the AC it gates (an all-skipped sweep is `blocked-runtime-evidence-missing`, never a deferral). The same law is mirrored into `POLICY['update-deps']`.
- **F2 — probe receipts in the structured return, enforced by correction-not-gate.** Each `terminal.deferrals[]` entry now carries a `probe` field (the literal command + one-line output tail). New pure helper `probeGaps()` surfaces ac-bearing deferrals lacking a receipt (deduped across `terminal.deferrals[]` and the sibling `residual[]` — a probe on *either* copy credits the AC). `driveVerify` turns a non-empty gap into exactly **one corrective re-run** demanding the receipt (or the evidence, if the probe shows the wall fell); still receipt-less after that → HARD-STOP. Crucially, `verifyClean`'s accept condition is **untouched** — the v9.114 lesson holds: never hard-gate a converged, defect-free slice on a formatting technicality. `collectDeferrals` threads the receipt through to the `/wf ship` block list.
- **F3 — prior deferrals are re-challenged, and the pile is made visible.** `orient()` now reads the open `runtime-evidence-deferrals` (`cleared-by: null`) from `00-index.md` into `ORIENT_RESULT.priorDeferrals`; `driveVerify` (and the update-deps exec) inject a **RE-CHALLENGE** block into the verify prompt — each prior wall is a claim to re-probe fresh, never a fact to inherit (the Crumb stale-creds incident that transiently nulled 70 users' link previews). The hand-back computes `outcome.deferralPressure` (`{ open, oldestDeferredAt, repeatWalls }`) and logs the oldest wall + repeat-of clusters, so a standing pile (the bot-backend pattern: 22–24 live-voice ACs deferred run after run) surfaces in every run summary instead of hiding inside artifacts. Visibility only — no new gate.
- **F6 — stage-scope clamp.** One operating-rule line in `runStage` and `runUpdateDepsExec`: the subagent's mandate is ONLY its own stage/slice — no running other stages, no claiming completion of other slices or the whole workflow, no routes beyond what its reference returns (the Isometric overclaim: an implement subagent asserting "all 9 slices complete… Next: /wf handoff" before the gating review ran).
- **Reference + tests.** `reference/yolo.md` updated to describe the receipt/corrective-re-run, the never-inherit re-challenge, and the new `deferralPressure` hand-back anchor. `tests/unit/skills/yolo-gates.test.mjs` extracts and covers `probeGaps`, `reChallengeClause`, and `deferralPressure` (12 new cases); all pre-existing `verifyClean`/`evaluateGate`/`collectDeferrals` cases pass unchanged.
...
Implements the same-day doc-site drift audit (`docs/internal/DOC-SITE-DRIFT-AUDIT-2026-07-12.md`), which found `docs/site/**` had accumulated ~33 releases (v9.89→v9.121) of drift since the 2026-06-20 pass — retired surfaces still taught as live, ~15 shipped features undocumented, schema-field gaps, and broken glossary anchors. ~52 findings applied across ~54 files:

- **Retired surfaces de-taught.** The sidebar's `↳ /review` nav label (the only sidebar entry naming a retired command) is relabeled `↳ /wf review` across all 51 pages + `nav.html` + the generator's `SIDEBAR`. `reference/wf.html` is rebuilt from a stale "10 stages + 4 add-on **keys** / 18 keys" framing to the correct **20-key** surface: the four augmentation *key* detail sections collapse into one "augmentations are shape-decided, not keys" section; `handoff`/`ship`/`retro` gain their `pr#N`/branch **batch** arg-signatures; new `yolo` section; `adopt` intake mode; the ship-plan-readiness pre-check; `steer.md`; and the verify regression-test mandate. Retired-term fixes (`amend`, `craft`, `init-ship-plan`-as-stage, augmentation-keys, standalone `/review`) swept across `how-to/` (use-design, close-workflows, author-ship-plan, use-augmentations, run-a-release), `orientation/mental-model`, `choose-a-command`, and `explanation/augmentations-model` (which taught the fully stale "an add-on is a command you run" model).
- **`whats-new.html` backfilled** v9.90→v9.122 (was frozen at v9.88 while the brand read v9.121 — the tell that triggered the audit).
- **Broken anchors (E1).** `reference/glossary.html`'s `<dt>` terms had zero `id=` attributes, silently breaking 17 cross-page links; added all anchors + a new `Handoff` entry (fixes a latent `#handoff` link the audit itself missed). Also corrected the `/wf design` sub-command count (20), the intake-mode count (nine, incl. `adopt`), and the `/wf review` framing.
- **Schema pages.** `00-index`/`08-handoff`/`09-ship-run` schema pages gained the missing frontmatter fields (`ship-plan-readiness`, `steering-honored`, batch `handoff-scope`/`handoff-lead`/`readiness-via`/`pr-readiness-verdict`/`ship-scope`/`shipped-via`, `revisions`, `provenance`, `adopt`), and the two inverted descriptions (`selected-slice-or-focus` priority, `plan-version-at-run` reads the integer `plan-version` counter) are corrected.
- **Reference coverage** added on skills (`study-sources`, restored `diataxis`; five non-`wf` skills), hooks (seven, incl. the two `leak-guard-*` guards + Grok/Cursor `normalizeHookPayload`), types/artifacts (`solution`, `ship-rollback`, the narrative `## The <Stage>` section), review (accumulating ledger + `pre-existing`), serve (branch grouping, `skipped-not-git`, `hub.lock.takeover`), tray (running-process self-heal + heartbeat + `SDLC_DISABLE_TRAY_HEAL`), build-and-dist (seven hooks, corrected `hooks:install`, `sync:codex`), faq (git precondition), and branch-strategy (`planned` liveness state).
- **Ground-truth + hygiene.** `skills/wf/reference/_fragment-authoring.md` Step F1 rich-tier list reconciled to `hooks/post-write-verify.mjs` `RICH_TIER_TYPES` (both trees); `README.md`'s dead `skills/review/SKILL.md` pointer repointed to `skills/wf/reference/review/`; `sunflower-view.md` (dead plan link, nonexistent `--path` flag, 11→14 fragment-type count); `docs/site/README.md` page count 42→21; deleted the two non-functional session-bound `_apply_*.py` scripts; added `docs/site/.gitignore` for the nested `.ai/_view/`.
- **Three audit false-findings verified against source and deliberately NOT applied:** the glossary/augmentations `rca`-vs-`profile` "contradiction" (the `augmentation-type` enum genuinely is `[benchmark, experiment, instrument, rca]` while shape's `augmentations-needed` genuinely is `[…, profile]` — two different valid lists); the "dead" `.github/workflows/sdlc-build-freshness.yml` CI reference (it exists at the repo root); and `craft` in the v9.82 changelog entry (correct as history).

Doc-site + reference-doc content only; `dist`/buildId move only from the version-string embed. Codex mirror regenerated from source via `sync:codex` (doc-site is in the shared runtime payload); `_fragment-authoring.md` mirrored by hand (skills tree is not in the payload).
...
- **Host provenance (main tree).** Render-queue `enqueuedBy.host` is now env-derived (`process.env.SDLC_HOST || 'claude'`) in both writer hooks (`hooks/render-on-artifact-write.mjs`, `hooks/session-start-orient.mjs`), matching the existing `lib/hub-lifecycle.mjs` pattern. The Codex adapter (`hooks/_adapter.mjs` `codexHostEnv()`) now sets `SDLC_HOST=codex` + `SDLC_HUB_STARTED_BY=codex` on every runtime spawn, so Codex-originated hubs and render work stop reporting as Claude-started. Claude-side behavior is byte-identical when the env is unset (new render-queue test covers both defaults).
- **Schema `$id` de-branding.** `tests/frontmatter.schema.json` and `schemas/sdlc-config.schema.json` `$id` moved from `anthropic.com` to the repo's own namespace (nothing dereferences the URL; synced into the Codex runtime payload).
- **Docs site.** Landing page gains the interim Codex host-syntax note ("read `/wf` as `$wf`") and a host-neutral lede; the full generator host-syntax toggle remains a scheduled separate workstream.
- **Codex tree (plugin 0.6.0 → 9.107.0).** Manifest rewritten for the real 8-skill/19-key surface (retired-router description dropped, CLI pin restated as "hooks-GA, verified on 0.143.0", `capabilities` → documented enum, `package.json` version lockstep, version now tracks the Claude plugin). Claudism punch list executed: `/compact` (2), stale router spellings (5 files), runtime-adapters de-Clauding, five directive `AskUserQuestion` sites replaced by the new `_gate-question.md` three-rung ladder (plan-mode `request_user_input` → numbered chat question → recorded policy default), 36 Bash-timestamp mandates single-sourced into `_timestamp.md`, Claude model pins (`haiku`/`sonnet`/`Task` call) replaced by `_subagents.md` effort tiering. New hooks: modern PreToolUse `permissionDecision: deny` envelope (exit-2 kept as fallback), `SubagentStart` context injection (minimal W6 landing site: active slug + External Output Boundary), `PermissionRequest` strict auto-allow of the plugin's own runtime invocations (logged). Subagent policy: read-only fan-outs (review dimensions, verify AC groups, intake research) are parallel-by-default `explorer` children in waves of ≤6, per `_subagents.md`.
- **Codex gates.** New `scripts/verify-claudisms.mjs` (7 scan families) wired ahead of `npm test`; new `scripts/verify-deployment.mjs` machine doctor (`npm run verify:deployment`) that catches the conformant-but-inert failure mode (the legacy v8.15.0 wrapper ran for a year while the native plugin was never installed — cutover completed 2026-07-08). New `plugins/sdlc-workflow-codex/.gitattributes` enforcing LF so the synced payload's buildId reproduces on Windows checkouts (125 runtime files renormalized).
...
- **Batch handoff.** Builds a roster of every slug on the branch, elects a stable `handoff-lead:` slug, and prints a roster report (per-slug stage / review verdict / open blockers / fingerprint / action) **before** packaging. Ready slugs are packaged (each writes its own `08-handoff.md`); not-ready slugs are reported and skipped without aborting their ready siblings. The PR/CI machinery runs **once** on the lead — the single shared PR's body is the **union** across packaged slugs and names any not-ready slug whose commits are on the branch. `pr-readiness-verdict` is the **logical AND** across the roster (a per-slug `ready` never makes the PR ready while a sibling's commits are unreviewed); ship gates on it. Followers carry `readiness-via: <lead>/08-handoff.md`. This also fixes the old shared-branch punt: in batch scope a `branch-strategy: shared` branch now creates its one PR.
- **Batch ship.** **All-or-nothing** across the branch (merge is atomic per PR): every roster slug must be `ready` (via the lead's `pr-readiness-verdict`) and clear its runtime-evidence deferral gate, or none ship. The 13-step sequence runs **once**, owned by the lead (`ship-scope: branch`, `branch-slugs`); each follower gets a `shipped-via: <lead>/09-ship-run-<run-id>.md` pointer row and advances to shipped with no duplicate run artifact. Changelog/release-notes and the announce phase aggregate across the whole roster.
- **`/wf status pr#N` roster view.** A read-only branch view (writes nothing beyond the registry reconcile) listing every slug on the branch and which are handoff-ready / ship-ready, resolving the branch from `INDEX.md`'s `branch` column.
- **Revision ledger replaces the in-body `## Revision N` pattern.** Revisable artifacts are now **living documents**: on re-invocation the body is rewritten to current truth and the *history of why* is recorded in a reason-centric frontmatter `revisions:` ledger (`{rev, at, trigger, because, changed}`) rendered as a timeline (shared `renderRevisionLedger` in `renderers/_history.mjs`, now used by `_simple.mjs` and `plan.mjs`), alongside the verbatim `history/` snapshots. A **no-op fingerprint guard** (`handoff-fingerprint`, and the general contract) skips re-packaging an artifact whose inputs are unchanged — the mechanism that makes batch re-runs cheap. The `_additive-write.md` single-source, its five citers (intake/shape/slice/handoff/retro), and `plan.md`'s former `## Revision History` sub-modes were all migrated.
- New `revisionLedger` schema `$def` + batch frontmatter fields (`handoff-scope`/`handoff-lead`/`handoff-fingerprint`/`readiness-via`/`branch-slugs`/`pr-readiness-verdict`; `ship-scope`/`branch-slugs`/`shipped-via`). New drift-guard test `tests/unit/skills/batch-handoff-ship.test.mjs` enforces the batch machinery + ledger model in both trees.
