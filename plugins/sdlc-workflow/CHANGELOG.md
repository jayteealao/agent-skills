...
- **SessionStart is conditional (W11.3).** `session-start-orient` enqueues a bootstrap render and spawns hub-ensure only when the project root has `.ai/workflows`, sits inside a git checkout, and the payload `source` is not `compact`; hub-ensure spawns on `startup` and `resume` only. The Codex `session-start` returns on `compact`. `lib/registry.mjs` `validateEntry` refuses roots under the OS temp dir, a Claude scratchpad, or `.claude/worktrees` (`ephemeralRootReason`; the test suite sets `SDLC_ALLOW_TEMP_ROOTS=1` in `tests/run-all.mjs`). On the operator machine on 2026-09-07 the eight `.ai/_view` directories in roots with no workflow store were deleted with their eight pending queue records: `Documents/dev`, `dev/agent-skills`, `dev/HomeLab-PDU-V1`, `dev/pi-unified`, `dev/cyberdeck`, `dev/Learn Electronics` (4 records), `dev/VIC3` (2 records), `Documents/rollover`; `Documents/dev` held 2 records.
