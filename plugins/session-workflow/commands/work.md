...
{Contracts/assumptions that must hold}
...
{List of constraints from plan/session}

### Guardrails

- MAX_CHANGED_FILES: {number}
- MAX_LOC_PER_STEP: {number}
- MUST_KEEP_MAIN_GREEN: {yes/no}
- FEATURE_FLAG_REQUIRED: {yes/no}
- MIGRATIONS_ALLOWED: {yes/no}
...
**Risk Notes:**
{Any behavioral changes, data shape changes, dependency changes}
{Or "No risk - additive change only"}
...
{Link to any decision notes created during work}
- [Decision: {title}](../decisions/decision-{date}-{slug}.md)
...
{Continue for all reviews in chain}
...
### Behavior Changes
{What changed for users, or "None - internal only"}

### Config Changes
{New env vars, settings, or "None"}

### Migration Steps
{Schema/data migrations, or "None"}

### Test Coverage Added
- Unit tests: {count} tests added
- Integration tests: {count} tests added
- E2E tests: {count} tests added
...
- [x] {Criterion 3} - ✅ Complete

**Status:** ✅ All acceptance criteria met

---

*Work log started: {YYYY-MM-DD}*
*Work log completed: {YYYY-MM-DD}*
