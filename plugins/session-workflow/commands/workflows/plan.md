...
- Presents next-step options via AskUserQuestion (deepen-plan, plan-review, workflows:work)
...
2. **Run `/workflows:deepen-plan`** - Enhance each section with parallel research agents (best practices, performance, UI)
3. **Run `/workflows:plan-review`** - Get feedback from reviewers (Senior, Simplicity, Framework Conventions)
...
- **`/workflows:deepen-plan`** - Call the /workflows:deepen-plan command with the plan file path to enhance with research
...
**Note:** If running `/workflows:plan` with ultrathink enabled, automatically run `/workflows:deepen-plan` after plan creation for maximum depth and grounding.

Loop back to options after Simplify or Other changes until user selects `/workflows:work` or `/workflows:plan-review`.
...
5. **After creation:**
   ...
   - Ask if they want to proceed to `/workflows:work` or `/workflows:plan-review`
